...
# Eventually, this utility will include all PDFs from https://cran.r-project.org/web/views/Distributions.html
# classes
# A - support is non-negative real numbers
# B - support is non-negative real numbers, shifted and transformed

# CHANGES:
#   - Updated the mv2par function for inverse-Gamma, Weibull, inverse-Weibull, and log-logistic 
#     distributions to solve for distribution parameters more directly.
#   - Updated pbetasr and pmgamma so that these CDF functions use the same XMIN and
#     XMAX as the PDF functions. TBD whether the XMIN and XMAX should come 
#     from global environment vs passed through function.

## Implemented new equations to solve for distribution parameters that do not involve
## optimization -- improves stability in results
...
pmgamma = function(q, shape, rate, lower.tail = TRUE, XMIN, XMAX) {
...
mgammaOBJ = list(name = "mgamma", initF = function(D) {
    ...
    pmgamma(tau, shape = p[1], rate = p[2], lower.tail = lt, XMIN = p[3], XMAX = p[4])
}, plotF = function(x, p, clr) {
...
invgamma_mv2p <- function(mn, vr) {
  try({
    F = function(alpha) mn^2/(alpha-2)-vr
    alpha <- uniroot(f = F, interval = c(2.1,300))$root # variance is undefined when alpha <= 2?
    beta = mn*(alpha-1)
...
lgnorm_mv2p = function(mn, vr) {
    try({
        ...
        xi = c(log(mn), sqrt(vr))
        est = try(abs(optim(xi, F, mn = mn, vr = vr)$par), silent = TRUE)
...
# approx solun for shape and scale per https://math.stackexchange.com/questions/1769765/weibull-distribution-from-mean-and-variance-to-shape-and-scale-factor
weibull_mv2p <- function(mn, vr) {
  try({
    F = function(k) vr/mn^2 - gamma(1+2/k)/(gamma(1+1/k))^2 + 1 
    k <- uniroot(f = F, interval = c(0.1,300))$root
    l = mn/(gamma(1+1/k)) # scale
    list(shape = k, scale = l)
  }, silent = T)
}
...
invweibull_mv2p <- function(mn, vr) {
  try({
    F = function(alpha) {
      mn^2*((gamma(1-2/alpha)/(gamma(1-1/alpha))^2) -1) - vr
    }
    alpha <- uniroot(f = F, interval = c(2.1,300))$root # variance is undefined when alpha <= 2
    beta = mn/gamma(1-1/alpha)
    list(shape = alpha, scale = beta)
  }, silent = F)
}
...
llogis_mv2p <- function(mn, vr) {
  try({
    F = function(beta) {
      b = pi/beta
      (2*mn^2*(sin(b))^2 )/(b*sin(2*b)) - mn^2 - vr
    }
    beta <- uniroot(f = F, interval = c(2.001,300))$root # variance is undefined when beta <= 2?
    b = pi/beta
    alpha = mn*sin(b)/b
    list(shape = beta, scale = alpha)
  }, silent = T)
}
...
qbetasr = function(p, shape1, shape2) { # update these to take XMIN and XMAX as args instead of using the global env
    qbeta((p - XMIN)/XMAX, shape1, shape2)/(XMAX - XMIN)
}
rbetasr = function(n, shape1, shape2) { # update these to take XMIN and XMAX as args instead of using the global env
...
betasrOBJ = list(name = "betasr", mv2par = betasr_mv2p, initF = function(D) {
    ...
    pbetasr(tau, shape1 = p[1], shape2 = p[2], lt, XMIN = p[3], XMAX = p[4])
}, plotF = function(x, p, clr) {
