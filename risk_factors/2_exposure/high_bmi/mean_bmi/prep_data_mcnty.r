...
combined_dt <- combined_dt[race != 1 & age < 98 & sex != 3 & level == "cbsa_mcnty"]

#### Load and merge SE estimates
...
combined_dt_80 <- combined_dt_80[, list(age = 80, obese_1 = weighted.mean(obese_1, weights, na.rm = TRUE),
                                 ...
                                 by = list(cbsa_mcnty_code, area, year, sex, race, level)]
...
###### Resort again
setkeyv(combined_dt, c("year", "sex", "race", "age", "cbsa_mcnty_code"))

message('save recodes')
recodes <- list()

combined_dt <- combined_dt[!is.na(cbsa_mcnty_code)]
...
combined_dt[, state_race := as.integer(as.factor(state_race_string))]
