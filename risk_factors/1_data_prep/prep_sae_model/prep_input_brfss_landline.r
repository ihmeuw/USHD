...
#data_combined[,bmi_report := weight/(height)^2]
#data_combined <- data_combined[bmi_report >= 10 & bmi_report <= 70, ]
...
overwt_draws <- paste("overwt", 1:length(bmi_draws), sep = "_")
underwt_draws <- paste("underwt", 1:length(bmi_draws), sep = "_")
obese_draws <- paste("obese", 1:length(bmi_draws), sep = "_")
...
# subset to landline/dual phone only
data_combined <- rbind(data_combined[year<2009], data_combined[year>2008 & phone_ushd %in% 2:3])
...
collapse_bmi <- function(DT, 
  ...
  if(is_prev){
    ...
    temp1 <- DT[, lapply(.SD, mean, na.rm =T), 
                .SDcols = cols,
                by = by_col]
    temp2 <- DT[, lapply(.SD, function(c) weighted.mean(c, w = weights, na.rm = T)),
                .SDcols = cols,
                by = by_col]
  }
...
data_combined_agg_mean <- collapse_bmi(data_combined_mean,
                                       cols = paste0("pred_bmi_", 1:10),
                                       weighted_cols_name = paste0("pred_bmi_weighted_", 1:10),
...
data_combined_agg_overwt <- collapse_bmi(data_combined_overwt,
                                         cols = paste0("overwt_", 1:10),
                                         weighted_cols_name = paste0("overwt_weighted_count_", 1:10),
                                         by_col = c("mcnty", "cbsa_mcnty_code", "state", "year", "sex", "race", "age_bin", "edu_code", "marital_ushd"),
                                         is_prev = T)
...
data_combined_agg_underwt <- collapse_bmi(data_combined_underwt,
                                          cols = paste0("underwt_", 1:10),
                                          weighted_cols_name = paste0("underwt_weighted_count_", 1:10),
                                          by_col = c("mcnty", "cbsa_mcnty_code", "state", "year", "sex", "race", "age_bin", "edu_code", "marital_ushd"),
                                          is_prev = T)
...
for (var in overwt_draws){
  ...
  data_combined_agg_obese <- collapse_bmi(data_combined_obese,
                                          ...
                                          by_col = c("mcnty", "cbsa_mcnty_code", "state", "year", "sex", "race", "age_bin", "edu_code", "marital_ushd"),
                                          is_prev = T)
  setnames(data_combined_agg_obese, c("sample_size", "weights"), c(paste0("sample_size_", n), paste0("weights_", n)))
...
data_combined_no_strat <- data_combined[!((is.na(mcnty) & is.na(cbsa) & is.na(cbsa_mcnty_code) & is.na(state)) | is.na(year) | is.na(sex) | is.na(race) | is.na(age_bin))]
...
##############################
measure_vars <- c(paste0("obese_", 1:10), paste0("underwt_", 1:10), paste0("overwt_", 1:10), paste0("pred_bmi_", 1:10))
...
year_agg <- direct_estimates[, c(list(year = -1,
                                      weights = sum(weights, na.rm = T),
                                      sample_size = sum(sample_size, na.rm = T),
                                      pop = sum(pop, na.rm = T),
                                      final_weight = sum(final_weight, na.rm = TRUE)),
                                 lapply(.SD, weighted.mean, w = final_weight, na.rm = T)),
                             .SDcols = measure_vars,
                             by = list(area, level, age_bin, sex, race, edu_code, marital_ushd)]

direct_estimates <- rbindlist(list(direct_estimates, year_agg), use.names = TRUE, fill = TRUE)

year_agg_no_strat <- direct_estimates_no_strat[, c(list(year = -1,
                                                        weights = sum(weights, na.rm = T),
                                                        sample_size = sum(sample_size, na.rm = T),
                                                        pop = sum(pop, na.rm = T),
                                                        final_weight = sum(final_weight, na.rm = TRUE)),
                                                   lapply(.SD, weighted.mean, w = final_weight, na.rm = T)),
                                               .SDcols = measure_vars,
                                               by = list(area, level, age_bin, sex, race)]
