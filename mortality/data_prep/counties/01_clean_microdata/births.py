...
def run_on_cluster():
    ...
    births_data_raw.to_csv('%s/parsed_data/births/_archive/%s/births_%s_parsed.csv' % (parent_dir, archive_date, year), index=False)  # archived version
    births_data_raw.to_csv('%s/parsed_data/births/births_%s_parsed.csv' % (parent_dir, year), index=False)

    # add NIDs here, which depend on year, for database tracking
    nids = pd.read_csv(parent_dir + 'FILEPATH/nvss_births_nids.csv')
    ...
    # 2. Save cleaned data
    births_data.to_csv('%s/clean_data/births/01_pre_imputation/_archive/%s/births_%s_unadjusted.csv' % (parent_dir, archive_date, year), index=False)  # archived version
    births_data.to_csv('%s/clean_data/births/01_pre_imputation/births_%s_unadjusted.csv' % (parent_dir, year), index=False)
