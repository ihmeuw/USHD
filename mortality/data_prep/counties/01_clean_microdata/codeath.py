...
def run_codeath(test=False):
    ...
    cod_data_raw.to_csv("FILEPATH", index=False)  # archived version
    cod_data_raw.to_csv("FILEPATH", index=False)

    # 2. Save missingness
    cod_missingness.to_csv("FILEPATH", index=False)  # archived version
    ...
    # save output
    cod_data.to_csv("FILEPATH", index=False)  # archived version
...
def parse_cod_mortality(year, parent_dir, us_indir, ps_indir, map_dir, nrows=None):
    ...
    if year < 1982:
        ...
        # For counties, we use a custom county mapping dataset
        ...
        #    1979; stick to 29186
        # -- Washabaugh, SD, has invalid FIPS of 46131 and 46001. Washabaugh had 46131 in 1982, merged with Jackson County in 1983
        ...
        # overlap multiple 1980s boroughs, and deaths here are reallocated proportionally to mean deaths in the 1980s
        # (see the script '01_adjust_alaska_nyc_virginia.R' in the folder "FILEPATH")
        ...
        # Anchorage mis-assigned FIPS of 02020, so we need to keep Yukon-Koyukuk intact until we redistribute it
    ...
    for county_type in ['res', 'occ']:  # repeat this process for state/county of residence and occurrence
        ...
        if year < 1982:
            ...
            # that territory to the special code '998', so we can identify the true 0s later in the cleaning process.
    ...
    # INDUSTRY AND OCCUPATION: not enough years to extract with confidence, but keep for now (see recode.py for details on this).
    # We only have data from 1985-1999, and just 1992-1999 data needs to be recoded. All other years, fill in with codes for 'unknown'.
    ...
    # to read in as strings in the future. We recode entity codes (the other codes on the certificate) the same way.
    # Range is determined by number of entity codes in dictionary (fewer in earlier years)
...
# uncomment to run on cluster via parallel_codeath.py
