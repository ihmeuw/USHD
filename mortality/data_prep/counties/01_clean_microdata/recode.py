    ...
    if state_res_code in ['PR', 'VI', 'GU', 'AS', 'MP']:
        ...
        if race_label in ['NH White alone', 'NH Black alone', 'NH AIAN alone']:
            ...
            if race_code == 0:
                ...
                print 'Warning: unrecognized race code {} in NH Multiracial, recoded to Unknown'.format(race_code)
        else:
            print 'Warning: unrecognized race label {}, recoded to NH Other Race'.format(race_label)
            recode = "NH Other Race"

    return recode

def recode_race_hisp(race_label):
  # We need to retain four categories of race/hispanic variables for the CoD team. This will be
  # done using the race group variable created above, which contains all relevant race information.
  # The following four groups are a broader generalization of the groups used by the US Counties team:
...
def recode_mother_age(age_var):
  ...
  if age_var in range(1, 10):
    ...
    print "Unexpected mother's age recode value: {}".format(age_var)
    recode = '999'
  else:
    recode = '999'
