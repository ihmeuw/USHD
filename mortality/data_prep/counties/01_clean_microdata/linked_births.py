...
Description: this is the main script for data cleaning. It consists of three functions:
              1. run_linked_births: run the data cleaning code within the current cluster session
              2. parse_linked_births: main function to clean data
...
def run_linked_births(test=False):
    ...
    lb_data_raw.to_csv("FILEPATH", index=False)

    # 2. Save missingness
    lb_missingness.to_csv("FILEPATH", index=False)  # archived version
    lb_missingness.to_csv("FILEPATH", index=False)
    ...
    # 3. Save cleaned data.
    lb_data.to_csv("FILEPATH", index=False)  # archived version
    lb_data.to_csv("FILEPATH", index=False)
