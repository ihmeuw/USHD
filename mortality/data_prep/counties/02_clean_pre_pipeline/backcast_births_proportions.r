...
## 25 births. This script is run after imputation and interpolation have been run.
##
## The years 2003-2005 are used to create proportions. Data are pooled across these three years for
## all proportions. These years are selected because a) they start right after the time period we
## apply the proportions to, b) we cannot pool data in years that contain a change-over from
## one birth certificate format or education encoding to another since that can lead to changes in
...
get_pre_backcasting_data <- function() {
  ...
  setnames(DT, "full_fips_res_numeric", "cnty") # must use county of residence, not occurrence
...
make_prop_2 <- function(data) {
  ...
  files <- file.info(list.files(paste0("FILEPATH"), full.names = TRUE))
...
make_prop_4 <- function(data) {
  ...
  DT[births_4_num == 0 & births_4_denom == 0, prop_4 := 0] # avoid NaN from division by 0
...
# Fill in proportions, depending on if the denominator is larger than min_sample_size. Fill in the
# proportion, numerator, and denominator, and note which level was used
...
make_births_education_props <- function(min_sample_size = 25, clean = T) {
  ...
  births_prop <- make_prop_1(data = births_data, years = c(2003:2005)) # NOTE the years selected
  ...
  # do this before selecting proportions! the denominator rule must still be applied.
...
apply_backcasting_props <- function(archive_date) {
  ...
  # clean up
  # explicitly dropping columns we don't need (NOTE: keep county_res for database)
  ...
  # Collapse education: this is necessary in order to apply the proportions and split all edu births
  # into our edu groups. Also collapse across geographic info about occurrence, but preserve geo
  # info about residence

  # NOTE: keep nid and county_res. Keeping county_res means that proportions, which are prepared at
  ...
  # NOTE that final only has 3 years in it. We never read in all the data
  ...
  # the years that were not backcasted (e.g., check that the columns in 2002 match 2003). Any two years
  # from the two groups will do.
  message("Checking that formats match...")
  test_2002 = fread("FILEPATH")
  test_2003 = fread("FILEPATH")
