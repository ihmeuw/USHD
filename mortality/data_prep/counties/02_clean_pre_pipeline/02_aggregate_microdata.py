"""
This script reads in each year's microdata file, compresses it to
get death counts, and reads it out in the format it should be in for the life
expectancy regression.

There are a few wrinkles in this process, one with misassigned deaths in
...
for year in yearvals:
    ...
    if include_race_ethn:
        ...
        file_name = "FILEPATH"
        terr_file_name = "FILEPATH"
    us.to_csv("FILEPATH")  # archived version
    us.to_csv("FILEPATH")

    if not territories.empty:
        territories = territories.drop(['state_fips'], axis=1)
        territories.to_csv("FILEPATH")  # archived version
        territories.to_csv("FILEPATH")
