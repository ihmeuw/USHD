...
get_cleaned_data <- function(file_path){
  ...
  # to save run time, these edu IDs were not updated here but rather converted in imputed_single_year.R.
...
################################################################################
# get filenames
root <- paste0(parent_dir, 'FILEPATH')
...
fwrite(edu_props, paste0(parent_dir, 'FILEPATH',
                         'births_edu_props.csv'),
       row.names = F)
