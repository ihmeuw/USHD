...
if("detach_spline_19_21" %in% ls(envir = .GlobalEnv) & "detach_spline_19_22" %in% ls(envir = .GlobalEnv)) {
  stop("Both 'detach_spline_19_21' and 'detach_spline_19_22' are specified. This doesn't make sense. Choose the one that is relevant for the years in your model!")
}
...
if (!resub & !type %in% c("post_models", "post_raking", "agg_raked_yll")) { # this section is skipped for resub or post_models/post_raking because it will have already been run
  ...
  message("dealing with pop_version / pop_file...")
...
# load all the cause hierarchy for all fatal causes
  causes <- get_cause_metadata(cause_set_id = 3, release_id = gbd_release_id) # reporting causes

  causes <- causes[is.na(yld_only), ]
  causes <- causes[!(is.na(yll_age_end) & is.na(yll_age_start))] # get rid of causes with no age information
  ...
  causes <- as.data.table(causes) # for some reason, we need a as.data.table() here, although this works just fine in Rstudio
  ...
  # We have to be a bit tricky about this because in GBD 2020 some causes start at age 2, which causes issues when matching against the object "ages"
  ## also have to deal with the fact that for edu, ages starts at 25, not zero, which contradicts many cause age ranges
  if (by_edu) {
  ...
  stopifnot(nrow(causes) == length(unique(causes$acause))) # loop won't work if this isn't true
  ...
  # exclude any additional causes specified [optional]
  if ("cause_exclusions.csv" %in% dir(dir)) {

    # load exclusions
    exclude <- fread(paste0(dir, "/cause_exclusions.csv"))

    # exclude parent cause (if just_children is F)
    causes[acause %in% exclude[just_children == F, acause], run_model := 0]

    # exclude any children (or grandchildren)
    for (cc in exclude$acause) {
      causes[grepl(causes[acause == cc, cause_id], path_to_top_parent) & acause != cc, run_model := 0]
    }
  ...
  # get cause-specific model information [optional] and then pre-compile all models
  ...
  for (this_cause in causes[order(level, cause_id), cause_id]) {
    ...
    if (causes[cause_id == this_cause, model] %in%
        c(
          ...
          "spline_iid_race_age_mcnty_two_indics_flexible",
          "spline_iid_race_age_yr_mcnty_two_indics"
        )) {
      if (this_cause == 696) {
        causes[cause_id == this_cause,
               covars :=  paste0("c(", paste(paste0("\"", c(
                 get("covars", .GlobalEnv), "indic"
               ), "\""), collapse = ","), ")")]
        ...
        causes[cause_id == this_cause,
               covars :=  paste0("c(", paste(paste0(
                 "\"", c(get("covars", .GlobalEnv), "indic_1", "indic_2"), "\""
               ), collapse = ","), ")")]
      }


    } else if (causes[cause_id == this_cause, model] %in%
               c("spline_indicator_covid")) {

      causes[cause_id == this_cause,
             covars :=  paste0("c(", paste(paste0("\"", c(get("covars",.GlobalEnv),"indic_20","indic_21"), "\""), collapse = ","), ")")]
  ...
  for (this_cause in causes[order(level, cause_id), cause_id]) {
    ...
    if(new_settings$V2[new_settings$V1 == "model"] %in%
       c("spline_iid_race_one_indic_flexible_re_covs",
         ...
         "spline_iid_race_age_mcnty_two_indics_flexible",
         "spline_iid_race_age_yr_mcnty_two_indics",
         "spline_iid_covid_year_slope_two_indics")) {
      ...
      if(this_cause == 696) {
        ...
        new_settings <-
          rbind(
            new_settings,
            data.table(V1 = "indicator_file", V2 =
                         "FILEPATH/indicator_2005_hurricane_deaths.rds")
          )
        # add a line for the indicator_year
        new_settings <-
          rbind(new_settings, data.table(V1 = "indicator_year", V2 = 2005))
        ...
        new_settings <-
          rbind(
            new_settings,
            data.table(V1 = "indicator_file", V2 =
                         "FILEPATH/indicator_2001_terrorism_deaths.rds")
          )

        # add a line for the indicator_year
        new_settings <-
          rbind(new_settings, data.table(V1 = "indicator_year", V2 = 2001))
        ...
        new_settings <-
          rbind(
            new_settings,
            data.table(V1 = "indicator_file_1",
                       V2 = "FILEPATH/indicator_2001_terrorism_deaths.rds")
          )

        #FILEPATHindicator_2005_hurricane_deaths_byrace.rds
        new_settings <-
          rbind(
            new_settings,
            data.table(V1 = "indicator_file_2",
                       V2 = "FILEPATH/indicator_2005_hurricane_deaths.rds")
          )

        # add a line for the indicator_year
        new_settings <-
          rbind(new_settings, data.table(V1 = "indicator_year_1", V2 = 2001))
        new_settings <-
          rbind(new_settings, data.table(V1 = "indicator_year_2", V2 = 2005))
    ...
    if(model %in% c("spline_iid_race_age_mcnty_two_indics_flexible",
                    "spline_iid_race_age_yr_mcnty_two_indics")) {

      new_priors <- c(new_priors, rep("c(5,0.05,-5)", 4))
    } else {
      new_priors <- c(new_priors, rep("c(5,0.05,-5)", 2))
    }

    new_priors <- paste(new_priors, collapse = ",")
    existing_prior <- new_settings[V1 == "prior_list", V2]
    new_prior <- gsub("\\)$",paste0(",",new_priors,"\\)"),new_settings[V1 == "prior_list", V2])

    new_settings$V2[new_settings$V1 == "prior_list"] <- new_prior

    if (by_edu & (causes[cause_id == this_cause, ages] == "c()")) {
      message("no ages available for cause_id {this_cause}, skipping...")
    ...
    for (age_spline_type in c("age_knots_spec")) {
      if (age_spline_type %in% new_settings$V1) {
        ...
        if(setequal(current_age_spline, c(20,25,45,65,85))) current_age_spline <- c(20,35,45,65,85)
        if(setequal(current_age_spline, c(40,45,65,85))) current_age_spline <- c(40,55,65,85)
...
jids = data.table()
## qsub _submit_single_cause.r for each cause ------------------------------------------------------
# loop through causes and submit
if (type != "prep_only") { # models_only, post_models, post_raking, all

  ## this is the JID for the all-cause submission script; we need to update this to the actual all-cause submission JID
  CLUSTER_NAME
  # if we do not submit all-cause for some reason, then this will just hold on "1" which isn't a hold, so the job will just run
  sub_all_hold <- 1

  for (this_cause in causes[order(level, cause_id), cause_id]) {
    subset = data.table()
    if (causes[J(this_cause), run_model] == 0 & type %in% c("models_only", "all")) next  # CONTINUE LOOP for post_models, post_raking
    ...
    subset[, jid:=sbatch(code = "_submit_single_cause.r",
                         name = job_name,
                         arguments = c(
                           paste0(dir, "/", cause_name, "/"), cause_name == "_all", type, resub, TRUE, all_cause_dependency, # just set testing to always be true so we get logs
    ...
    Sys.sleep(5) # pause so as not to overload the scheduler
