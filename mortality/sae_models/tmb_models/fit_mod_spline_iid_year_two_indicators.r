...
##              u6_{a} ~ IID(sigma_6)
##              u7_{a} ~ IID(sigma_7)
##              where j = area, a = age group, t = year, r = race, S(a') = age spline, S(t') = time spline, 
##                  D = deaths, P = population, m = mx, I = indicator
##
...
TMB::compile("tmb_models/mod_spline_iid_year_two_indicators.cpp")

dyn.load(dynlib("tmb_models/mod_spline_iid_year_two_indicators"))
config(tape.parallel = 0, DLL = "mod_spline_iid_year_two_indicators")

# make objective function
cat("\n\n***** Make objective function\n"); flush.console()
obj <- MakeADFun(tmb_data, tmb_par, random = c("B", paste0("re", 1:7)), 
                 DLL = "mod_spline_iid_year_two_indicators", map = map)
...
se_time <- proc.time()
saveRDS(obj$report(),  file = paste0(dir, "/model_fit_OPT_", sex, "_", race, "_", edu, ".rds")) 
