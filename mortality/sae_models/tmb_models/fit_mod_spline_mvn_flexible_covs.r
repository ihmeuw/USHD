...
##                 + u4_{r}*X1[1] + u5_{r}*X1[2] + u6_{r}*X1[3]
##              u1_{j,a',t',r} ~ LCAR:LCAR:LCAR:MVNORM(rho_1j, rho_1a, rho_1t, rho_1j, sigma_1, cor_1)
...
##
## Note:        u4, u5, and u6 are created with gen_covar_subpop_log_sigmas(covar_subpop_hyperpriors_settings). There
##                can be any number of these, but in this example we have 3 of them because there are 3 subpopulation-
##                specific covariates.
##              X1 is the matrix of subpopulation-specific covariates (e.g. race/ethnicity-specific covariates)
##
########################################################################################################################################################
...
tmb_data <- list(
  ...
  # all covariate random effects are being given the same hyperpriors parameters
...
tmb_par <- list(
  ...
  re1_log_sigma = rep(re1_log_sigma, num_r),
  logit_rho_1j = 0,
  logit_rho_1t = 0,
  logit_rho_1a = 0,
  cor_1 = rep(0, num_r * (num_r - 1) / 2),
  ...
  re3 = array(rep(0, num_t * num_a * num_r), dim = c(num_t, num_a, num_r)),
...
TMB::compile("tmb_models/mod_spline_mvn_flexible_covs.cpp")

dyn.load(dynlib("tmb_models/mod_spline_mvn_flexible_covs"))
config(tape.parallel = 0, DLL = "mod_spline_mvn_flexible_covs")

# make objective function
cat("\n\n***** Make objective function\n"); flush.console()
obj <- MakeADFun(tmb_data, tmb_par, random = c("B", paste0("re", 1:3),"covar_subpop_re_matrix"), 
                 DLL = "mod_spline_mvn_flexible_covs", map = map)
