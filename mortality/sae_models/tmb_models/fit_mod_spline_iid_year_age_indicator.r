...
tmb_data <- list(
  ...
  T = data$year[ii],
  R = data$race[ii],
  I = data$indic[ii], # indicator for shocks
...
tmb_par <- list(
  ...
  re5 = rep(0, num_a), # age-level random intercept (IID) to be multiplied by the indicator
...
TMB::compile("tmb_models/mod_spline_iid_year_age_indicator.cpp")

dyn.load(dynlib("tmb_models/mod_spline_iid_year_age_indicator"))
config(tape.parallel = 0, DLL = "mod_spline_iid_year_age_indicator")

# make objective function
cat("\n\n***** Make objective function\n"); flush.console()
obj <- MakeADFun(tmb_data, tmb_par, random = c("B", paste0("re", 1:5)), 
                 DLL = "mod_spline_iid_year_age_indicator", map = map)
