// see fit_mod_spline_iid_race_one_indic_flexible_re_covs.r for model description
...
template<class Type>
  Type objective_function<Type>::operator() () {
    ...
    DATA_IVECTOR(I);    // indicator of shock
    ...
    DATA_INTEGER(num_subpop); // dimension of subpopulation
    ...
    matrix<Type> Sigma1(num_subpop,num_subpop); // cannot use sigma1.size() because sigma 1 here only has one dimension
    Sigma1.setIdentity();
    ...
    for(size_t i = 0; i < Y.size(); i++) {
      ...
      for (size_t k = 0; k < covar_subpop_re_matrix.cols(); k++) {
        log_m[i] += covar_subpop_re_matrix(R[i], k) * X1(i, k); // add the jth covariate-RE product
      } // end k

      // add in RE2 and RE3
      log_m[i] += re2[J[i]] + re3(T[i], A[i], R[i]) + re4[J[i]]*I[i] +
        re5[A[i]]*I[i];
