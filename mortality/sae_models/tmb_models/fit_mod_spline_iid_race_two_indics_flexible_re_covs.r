...
##              log(m_{j,a,t}) = B0 + B*X_{j,t} +  B*X1_{j,t,r} + u1_{j,a',t',r}*S(a')*S(t') + u2_{j} + u3_{a,t,r} +
##                u4_{j}*I + u5_{j}*I + u6_{a}*I + u7_{a}*I + u8_{r}*X1[j,t,r]
...
##              u6_{a} ~ IID(sigma_6)
##              u7_{a} ~ IID(sigma_7)
##              u8_{r} ~ IID(sigma_subpop) [vector of IID random effects multiplied by each R/E specific covariate]
...
##              indicator file specified in settings.csv
##
...
tmb_data <- list(
  ...
  I_first = data$indic_1[ii], # indicator for shocks
  I_second = data$indic_2[ii], # indicator for shocks
  graph_j = graph_j,
  graph_t = graph_t,
  graph_a = graph_a,
  s_age = s_age,
...
tmb_par <- list(
  ...
  re4 = rep(0, num_j), # area/age-level random intercept (IID) to be multiplied by the indicator
  re4_log_sigma = re4_log_sigma,
  
  re5 = rep(0, num_j), # area/age-level random intercept (IID) to be multiplied by the indicator
  re5_log_sigma = re5_log_sigma,
  
  re6 = rep(0, num_a), # area/age-level random intercept (IID) to be multiplied by the indicator
  re6_log_sigma = re6_log_sigma,
  
  re7 = rep(0, num_a), # area/age-level random intercept (IID) to be multiplied by the indicator
...
TMB::compile("tmb_models/mod_spline_iid_race_two_indics_flexible_re_covs.cpp")

dyn.load(dynlib("tmb_models/mod_spline_iid_race_two_indics_flexible_re_covs"))
config(tape.parallel = 0, DLL = "mod_spline_iid_race_two_indics_flexible_re_covs")
...
obj <-
  MakeADFun(
    tmb_data,
    tmb_par,
    random = c("B", paste0("re", 1:7), "covar_subpop_re_matrix"),
    DLL = "mod_spline_iid_race_two_indics_flexible_re_covs",
    map = map
  )
