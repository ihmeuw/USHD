// see fit_mod_spline_iid_edu_covs_offset_shocks_indicator.r for model description
...
template<class Type>
  Type objective_function<Type>::operator() () {
    ...
    DATA_IVECTOR(E);    // edu indicator
    DATA_MATRIX(X1);    // edu-ethnicity specific covariates
    DATA_MATRIX(X_offset); // offset covariate (not edu specific, only state and year specific)
    DATA_IVECTOR(S);    // state indicator
    DATA_IVECTOR(I_first);    // indicator of if a year is 2001 or not
    DATA_IVECTOR(I_second);    // indicator of if a year is 2005 or not
    ...
    PARAMETER_VECTOR(B); // corresponds to data matrix X
    PARAMETER_VECTOR(B_offset); // corresponds to data matrix X_offset

    // RE1: area/edu/age spline/time spline effect
    PARAMETER_ARRAY(re1);
    ...
    // RE3: age-year-edu-level random intercept (LCAR:LCAR:MVNORM)
    PARAMETER_ARRAY(re3);
    ...
    // covar offset covariates
    PARAMETER_ARRAY(covar_offset_random_effect);
    PARAMETER(covar_offset_log_sigma);
    Type sigma_offset = exp(covar_offset_log_sigma);
    DATA_STRUCT(covar_offset_hyperpriors_parameters, prior_type_sigma);

    // NLL contribution from random effects
    Type nll = 0;
    max_parallel_regions = omp_get_max_threads();

    // RE1: area/edu/age spline/time spline effect random effect
    ...
    matrix<Type> Sigma(num_subpop,num_subpop); // cannot use sigma1.size() because sigma 1 here only has one dimension
    Sigma.setIdentity();
    Eigen::SparseMatrix<Type> Sigma_Sparse=asSparseMatrix(Sigma);
    REPORT(Sigma_Sparse);

    // RE1: edu(area(age_spline,year_spline))
    ...
    // RE3: age-year-edu-level random intercept
    SparseMatrix<Type> K_3t = lcar_strmat(graph_t, rho_3t);
    SparseMatrix<Type> K_3a = lcar_strmat(graph_a, rho_3a);

    // identity matrix for edu
    matrix<Type> Sigma1(num_subpop,num_subpop);
    Sigma1.setIdentity();
    ...
    // Area/age-level IID random intercepts (related to shocks indicator variables)
    ...
    // Random effect on the edu/ethnicity specific covariates
    // covar_subpop_re_matrix has a column for each covariate, and a row for each edu
    ...
    // Random effect on the offset covariate
    // covar_offset_random_effect has dimensions for education, age, and state: 4x13x51
    PARALLEL_REGION nll -= dnorm(vector<Type>(covar_offset_random_effect), Type(0), sigma_offset, true).sum();
    ...
    // pen for the offset covariate
    Type pen_offset = eval_prior_sigma(covar_offset_hyperpriors_parameters, covar_offset_log_sigma);
    PARALLEL_REGION nll -= pen_offset;
    ...
    // predictions
    // start constructing log(mx) with the intercept and covariates with fixed effects
    vector<Type> log_m = X * B;

    // loop over rows in the dataset to finish constructing log(mx)
    for(size_t i = 0; i < Y.size(); i++) {

      log_m[i] += B_offset[E[i]] * X_offset(i);

      // add in the covariates w/ random effects
      for (size_t k = 0; k < covar_subpop_re_matrix.cols(); k++) {
        log_m[i] += covar_subpop_re_matrix(E[i], k) * X1(i, k); // add the jth covariate-RE product
      } // end k

      // add in RE2 and RE3
      log_m[i] += re2[J[i]] + re3(T[i], A[i], E[i]) +
        re4[J[i]]*I_first[i] +
        re6[A[i]]*I_first[i]+
        re5[J[i]]*I_second[i] +
        re7[A[i]]*I_second[i];

      // add in covar offset covariate random effect
      log_m[i] += covar_offset_random_effect(E[i], A[i], S[i]) * X_offset(i);

      // add in RE1
      for (size_t tp = 0; tp < s_year.cols(); tp++) {
        for (size_t ap = 0; ap < s_age.cols(); ap++) {
          log_m[i] += re1(tp, ap, J[i], E[i]) * s_year(T[i],tp) * s_age(A[i],ap);
        }
      }
    }
