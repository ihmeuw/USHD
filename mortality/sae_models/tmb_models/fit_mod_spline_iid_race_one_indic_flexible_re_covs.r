...
tmb_par <- list(
  ...
  # area-level random intercept (IID) to be multiplied by the indicator
  re4 = rep(0, num_j),
  re4_log_sigma = re4_log_sigma,
  
  # age-level random intercept (IID) to be multiplied by the indicator
  re5 = rep(0, num_a),
...
TMB::compile("tmb_models/mod_spline_iid_race_one_indic_flexible_re_covs.cpp")

dyn.load(dynlib("tmb_models/mod_spline_iid_race_one_indic_flexible_re_covs"))
config(tape.parallel = 0, DLL = "mod_spline_iid_race_one_indic_flexible_re_covs")
...
obj <-
  MakeADFun(
    tmb_data,
    tmb_par,
    random = c("B", paste0("re", 1:5), "covar_subpop_re_matrix"),
    DLL = "mod_spline_iid_race_one_indic_flexible_re_covs",
    map = map
  )
