...
##              edu [integer] -- edu to generate predictions for
...
##              resub [boolean] -- if running with resubmission, check if the final files are present
##
## Requires:    mx draws saved by subdraw for the year of interest:
##                [dir]/mx_est_[area_var]_[year]_[sex]_[race]_[edu]_[max_subdraw].rds
##
## Outputs:     mx draws and estimates, saved in separate files by year:
##                "[dir]/mx_draws_[area_var]_[year]_[sex]_[race]_[edu].rds"
##                "[dir]/mx_est_[area_var]_[year]_[sex]_[race]_[edu].rds"
##
####################################################################################################
...
save_draws <- function(draws, race_together_temp, edu_together_temp, expected_files, y, r=NULL, e=NULL) {
  ...
  if (race_together_temp) {
    ...
    saveRDS(draws[race == r, ], file = paste0(dir, "/mx_draws_", area_var, "_", y, "_", sex, "_", r, "_", edu, ".rds"))
    saveRDS(est[race == r, ], file = paste0(dir, "/mx_est_", area_var, "_", y, "_", sex, "_", r, "_", edu, ".rds"))
    
  } else if (edu_together_temp) {
    
    saveRDS(draws[edu == e, ], file = paste0(dir, "/mx_draws_", area_var, "_", y, "_", sex, "_", race, "_", e, ".rds"))
    saveRDS(est[edu == e, ], file = paste0(dir, "/mx_est_", area_var, "_", y, "_", sex, "_", race, "_", e, ".rds"))
    
  } else {
    saveRDS(draws, file = paste0(dir, "/mx_draws_", area_var, "_", y, "_", sex, "_", race, "_", edu, ".rds"))
    saveRDS(est, file = paste0(dir, "/mx_est_", area_var, "_", y, "_", sex, "_", race, "_", edu, ".rds"))
...
if ((!race_together & by_race) | (!edu_together & by_edu)) {
  ...
  final_file <- paste0("mx_draws_", area_var, "_", this_year, "_", sex, "_", race, "_", edu, ".rds")
  ...
  expected_files <- paste0("mx_draws_", area_var, "_", this_year, "_", sex, "_", race, "_", edu, "_",
  ...
  if(!is.null(all_draws)) {
    save_draws(all_draws, race_together, edu_together, expected_files, y=this_year, r = if (by_race) race, e = if (by_edu) edu)
  }
  ...
  for (r in races) {
    for (e in edu_groups) {
      ...
      # if resubmitting and the final file is already present, then we don't need to re-compile
      final_file <- paste0("mx_draws_", area_var, "_", this_year, "_", sex, "_", r, "_", e, ".rds")
      ...
      expected_files <- paste0("mx_draws_", area_var, "_", this_year, "_", sex, "_", r, "_", e, "_",
                               sub_draws+(draw_width-1), ".rds")
      ...
      if(!is.null(all_draws)) {
        save_draws(all_draws, race_together, edu_together, expected_files, y=this_year, r=r, e=e)
      }
