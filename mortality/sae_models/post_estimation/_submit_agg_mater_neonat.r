####################################################################################################
## Description: Launches agg_mater_neonat.r as an array job because otherwise it runs very slowly
...
# settings
script <- paste0("/post_estimation/agg_mater_neonat.r")
queue <- "QUEUE"
fthread <- 2
m_mem_free <- "80G"
h_rt <- "05:00:00"
...
write.csv(combos, paste0(dir, "/mater_neonat/", "/model_args.csv"))

run_at_once <- 80

id <- sbatch(code = script,
             arguments = c(dir, resub),
             name = paste0("agg_mater_neonat"),
             fthread = fthread, 
...
sbatch(code = fs::path_package("sae.shared", "scripts", "compile_estimates.r"),
     arguments = c(paste0(dir,"/mater_neonat/"), raked, lt, validate, agg_skip_flags),
     hold = id, 
     fthread = 2, m_mem_free = paste0(mem_comp,"G"), h_rt = "04:00:00",
     archive = archive,
     project = project, queue = queue,
     sgeoutput = paste0(dir, "/mater_neonat/"))
