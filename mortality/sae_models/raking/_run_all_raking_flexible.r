...
#If you've cleared the global environment with rm(list=ls()), jobmon doesn't reload.
if ("jobmonr" %in% .packages()) {
...
if (interactive()) {
  ...
  from_geo <- 'mcnty'
  to_geo <- 'mcnty'
  common_raking_vars <- 'year,age,sex,sim,mcnty' # for an all-cause mx rake, all-race: 'year,sex,age,sim'; by-race: 'year,sex,age,sim'
  
  lower_raking_vars <- 'year,age,sex,sim,edu,mcnty' # for an all-cause mx rake, all-race: 'year,sex,age,sim,mcnty'; by-race: 'year,sex,age,sim,mcnty,race'
  upper_raking_vars <- 'year,age,sex,sim,mcnty' # for an all-cause mx rake, all-race: 'year,sex,age,sim,state'; by_race: 'year,sex,age,sim,mcnty'
  initial_level <- NULL
  terminal_level <- NULL
  project <- "PROJECT"
  queue <- "QUEUE"
...
if (terminal_level>0) {
  # We need some minor string parsing to pass arguments to the 2-D raking script correctly.
  ...
  #       this will require first writing OUTPUTS to said to_dir in the rake_by_.._re.r scripts
...
for (lvl in initial_level:terminal_level) {
  ...
  if (lvl == 0) {
    ...
    if (nrow(raking_jobs[level == lvl - 1])>0){
      ...
      # integers like draws/years/sex need to be cast to int explicitly, as reticulate only considers
      ...
      raking_jobs[
        ...
        `:=` (job_id = list(jobmon_add_task(jobmon_task_template = task_templates$flexible_raking_template,
                                            ...
                                            archive = T,
                                            sgeoutput = paste0(from_dir, "FILEPATH"), queue = queue,
                                            # task args
                                            from_dir = from_dir,
                                            to_dir = to_dir,
                                            measure = measure,
    ...
    raking_jobs[
      ...
      `:=` (job_id = list(jobmon_add_task(jobmon_task_template = task_templates$flexible_2d_raking_template,
                                          ...
                                          common_raking_vars = common_raking_vars_2d, # Special case.
                                          draws = as.integer(draws),
                                          # convert logical to character to pass through jobmon
...
notification = ifelse(measure=="yll", "YLL raking", "Mortality rate raking")
send_email(notification, project, queue)
