...
for (lvl in initial_level:terminal_level) {
  ...
  if (lvl == 0) {
    ...
    if (nrow(raking_jobs[level == lvl - 1])>0){
      ...
      raking_jobs[
        level == lvl - 1, 
        `:=` (job_id = list(jobmon_add_task(jobmon_task_template = load_jobmon_task_templates(jobmon_tool)$flexible_raking_template,
                                            tool = jobmon_tool,
                                            name = job_name, 
    ...
    raking_jobs[
      level == lvl - 1, 
      `:=` (job_id = list(jobmon_add_task(jobmon_task_template = load_jobmon_task_templates(jobmon_tool)$flexible_2d_raking_template,
                                          tool = jobmon_tool,
                                          name = job_name, 
    ...
    if (nrow(compile_jobs)>0){
      ...
      compile_jobs_with_holds[
        level == lvl, 
        `:=` (job_id = list(jobmon_add_task(jobmon_task_template = load_jobmon_task_templates(jobmon_tool)$raking_compile_template,
                                            tool = jobmon_tool,
                                            name = job_name, 
