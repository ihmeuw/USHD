...
calc_growth_rate <- function(dir, test_by_race, test_by_edu, n_years = 10, years, pop_growth_file, area_var) {
  ...
  # is weighted by the race-specific 85+ population 
  
  pop_full <- readRDS(pop_growth_file) 
  ...
  # reset tiny populations to 0 to avoid edge cases where one group gets all of the weight
