...
# validation is about if this is for a validation run or not. It doesn't mean
# anything about validating settings
check_settings <- function(dir, validation = F) {

  # check that the main directory exists and is on FILEPATH
  ...
  if (class(detach_2019_2020) != "logical") stop("'detach_2019_2020' should be type logical")
  if (class(detach_2020_2021) != "logical") stop("'detach_2020_2021' should be type logical")
  if (class(detach_2021_2022) != "logical") stop("'detach_2021_2022' should be type logical")
  if (class(detach_spline_19_21) != "logical") stop("'detach_spline_19_21' should be type logical")
  if (class(detach_spline_19_22) != "logical") stop("'detach_spline_19_22' should be type logical")
  ...
  if (length(ages) == 1 & !model %in% c("1b", "2b", "2b_edu", "spline_no_age_no_race_eth_cov", "spline_no_age_edu")) stop(paste("Model", model, "is only for use with multiple age groups (i.e., 'ages' is length 2+)"))
  ...
  if("NH Multiracial" %in% race_labels) {

    stopifnot(max(races) == races[which(race_labels == "NH Multiracial")]) # make sure NH Multiracial is the max index because the offset code requires this
    stopifnot(which(race_labels == "NH Multiracial") == length(race_labels)) # make sure NH Multiracial is listed last in the race_labels bc the offset code prediction requires this
  ...
  if(detach_spline_19_21 & is.null(year_knots_spec)) stop("'detach_spline_19_21' is TRUE but 'year_knots_spec' is NULL")

  if(detach_spline_19_21 & !(2019 %in% year_knots_spec & 2020 %in% year_knots_spec & 2021 %in% year_knots_spec)) {
    stop("'detach_spline_19_21' is TRUE, but 2019, 2020, or 2021 are not specified in the year knot placements")
  }

  if(detach_2019_2020 & !(2019 %in% years & 2020 %in% years)) stop("'detach_2019_2020' is specified, but 2019 or 2020 are not in the model")
  if(detach_2020_2021 & !(2020 %in% years & 2021 %in% years)) stop("'detach_2020_2021' is specified, but 2020 or 2021 are not in the model")
  if(detach_2021_2022 & !(2021 %in% years & 2022 %in% years)) stop("'detach_2020_2021' is specified, but 2021 or 2022 are not in the model")
    ...
    for (type in val_types) {
      ...
      for (size in format(val_sizes, scientific = F, trim = T)) {
        for (it in 1:val_iter) {
          deaths_file_tmp <- paste0(val_dir, "/", type, "/deaths_", size, "_", it, ".rds")
          pop_file_tmp <- paste0(val_dir, "/", type, "/pop_", size, "_", it, ".rds")
          if (!file.exists(deaths_file_tmp)) stop(paste0("deaths file", deaths_file_tmp,  "does not exist in 'val_dir'/'val_types'"))
          if (!file.exists(pop_file_tmp)) stop("pop file", pop_file_tmp,  " does not exist in  'val_dir'/'val_types'")
        }
      }
    }
  }
  ...
  if(!is.null(pop_file)) {
    ...
    # pop won't have every county (tag: square)
    ...
    if (!validation & sum(!years %in% pop$year)) stop("not all years in 'years' are present in 'pop_file'")
  ...
  if (!is.null(covars) & is.null(covar_versions)) {
    if(length(covars[!(covars %in% c("indic","indic_1","indic_2"))]) > 0) {
      stop("covariates specified but there is not a covariates version list specified")
    }
  }
  if (!is.null(covars)) {

    # validate mcnty level covariates
  ...
  if (!is.null(covars_as)) {
    ...
    # validate each age level covariate
    for(cc in covars_as) {
  ...
  # Validate each subpop level covariate
  if (!is.null(covars_subpop)) {
    ...
    for (cc in covars_subpop) {
      ...
      all_vars <- c(area_var, "year", ifelse(by_race, "race", "edu"), cc) # this means that the covar_short_name has to be a column in covar
      if (sum(!years %in% covar$year)) stop(paste0("not all years in 'years' are present in subpop ",cc," in DB"))
      if (!(cc == "race_ethn_by_edu")) { # this test doesn't make sense for by_edu and this covariate
  ...
  if (!is.null(covars_trans)) {
    ...
    # reading mcnty-level covariates
    if(!is.null(covars)) {
      ...
      for (cc in covars[!(covars %in% c("indic","indic_1","indic_2"))]) {
        ...
        if(nrow(cov_all) == 0) stop("cov_all (covars read loop) has zero rows")

      }

      covar[[1]] <- cov_all[, (c("mcnty", "year", covars[!(covars %in% c("indic", "indic_1", "indic_2"))])), with = F]

    }

    # reading age-sex level covariates
    if(!is.null(covars_as)) {
      ...
      for(cc in covars_as) {
        ...
        if(nrow(cov_all) == 0) stop("cov_all (covars_as read loop) has zero rows")
    ...
    # reading subpop level covariates
    if (!is.null(covars_subpop)) {
      ...
      for (cc in covars_subpop) {
        ...
        # test that we got a subpopulation covariate, not a county level covariate:
        if (!length(unique(c(unique(cov_tmp$edu), unique(cov_tmp$race)))) > 1) {
          stop(
            paste0(
              "covariate ",
              cc,
              " is not a subpopulation covariate, but a county level covariate. Please check the covariate_dataset_id: ",
              as.numeric(covar_subpop_versions[cc]),
              ". It should have more than one unique value of race or education. Exiting."
            )
          )
        }


        # merge with other subpop covariates
        if (nrow(cov_all) == 0) {
          cov_all <- copy(cov_tmp)
        } else {
          cov_all <- merge(cov_all, cov_tmp, by = cov_merge_vars)
        ...
        if(nrow(cov_all) == 0) stop("cov_all (covars_subpop read loop) has zero rows")


      } # end loop over all covars_subpop
    ...
    #covar <- lapply(c(covar_file, covar_as_file, covar_subpop_file), readRDS)
  ...
  if (fixed_age_time) {
    ...
    # if (by_edu) if (length(setdiff(edu_groups, est$edu))) stop("'ref_est_file' does not include all education groups in 'edu_groups'")
    rm(est); gc()
  }
