...
check_settings <- function(dir, validation = F) {
  ...
  # check that n.sims > 0 but not super large (causes memory issues)
  ...
  # make sure that the Limited Use file path exists
  if (!dir.exists(paste0("FILEPATH",LU_folder))){
    stop(paste0("The LU file path you specified does not exist: ",paste0("FILEPATH",LU_folder)))
  }
  ...
  if (!is.null(covars)) {
    ...
    all_vars <- all_vars[all_vars != "low_mx_age"] # exclude low_mx_age bc that is added in prep_inputs.r
  ...
  if (!is.null(covars_subpop) & is.null(covar_subpop_file)) stop("race-ethnicity or education covariates specified but there is not a race-ethnicity/education covariates file specified")
  ...
  if(!is.null(covars_subpop)) {
    covar <- readRDS(covar_subpop_file)
    if (!class(covar)[1] == "data.table") stop("object in 'covar_subpop_file' should be a data.table")
    all_vars <- c(area_var, "year", ifelse(by_race, "race", "edu"), covars_subpop)
    miss <- setdiff(all_vars, names(covar))
    if (length(miss)) stop(paste("Variables missing from 'covar_subpop_file':", paste(miss, collapse = "; ")))
    if (sum(!years %in% covar$year)) stop("not all years in 'years' are present in 'covar_subpop_file'")
    if (sum(!races %in% covar$race)) stop("not all race/ethnicity groups in 'races' are present in 'covar_subpop_file'")
    if (length(unique(covar[[area_var]])) != num_j) stop("there are a different number of counties in 'covar_subpop_file' than in 'adjmat_file'")
    if (sum(!unique(covar[[area_var]]) %in% 0:(num_j - 1))) stop("area variable is not coded correctly in 'covar_subpop_file' (i.e., sequentially from 0)")
    if (sum(is.na(covar[year %in% years, all_vars, with = F]))) stop("NAs in 'covar_subpop_file' for specified years")
    rm(covar); gc()
  }
  ...
  if (!is.null(covars_trans)) {
    ...
    if(!is.null(covars_subpop)) {
      covar_subpop_temp <- readRDS(covar_subpop_file)
      if(!("race" %in% names(covar_subpop_temp))) covar_subpop_temp[,race := all_pop_id]
      if(!("edu" %in% names(covar_subpop_temp))) covar_subpop_temp[,edu := all_pop_id]
      covar[[length(covar) + 1]] <- covar_subpop_temp[,(c("mcnty","year","race","edu",covars_subpop)), with=F]
    }
  ...
  if(lt_hc) {
    if(is.null(pop_growth_file)) stop("lt_hc is TRUE, but you have not specified pop_growth_file")
  ...
  if(!lt_hc & !is.null(pop_growth_file)) stop("You have specified a pop_growth_file but lt_hc is FALSE")
