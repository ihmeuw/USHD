...
def adjust_dsp_liver_cancer(df, code_system_id, code_map_version_id):
    ...
    # handling for code system 40, icd9 dsp
    ...
    # handling for ICD9 detail (just hong kong and Macao)
    # there are no other locs in the ICD9 detail data, but specify them anyway to be safe
    elif code_system_id == 6:
...
class PoliceConflictCorrector:
    ...
    def _get_package_code_ids(cause_map: pd.DataFrame, **cache_kwargs) -> pd.DataFrame:
        ...
        return (
            ...
            # we only use four-digit ICD codes in redistribution
            .pipe(
                engine_room.remove_five_plus_digit_icd_codes,
                "garbage_code",
                code_system_id=code_system_id,
            )
            # only ICD9/10 data = remove_decimal is always True
            .assign(value=lambda d: misc.clean_icd_codes(d.garbage_code, True))
            # packages can contain the same garbage code twice, especially after cleaning
            .drop_duplicates("value")
    ...
    def get_deficit(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # filter the crosswalk to create observations only where we could have
        # had police conflict observations (e.g. only keep one year of the
        # crosswalk if we are only processing one year)
