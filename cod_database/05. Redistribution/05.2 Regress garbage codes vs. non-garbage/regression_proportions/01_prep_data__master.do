	...
	** ****************************************************************
		...
		// Location set version
			local location_set_version `1'
			
		// Input data name
			local input_data_name "`2'"
		...
		// Pull location hierarchy
			// Get data from hierarchy
				odbc load, exec("SELECT * FROM ADDRESS WHERE location_set_version_id = `location_set_version'") dsn(ADDRESS) clear
	...
	// Standard ICD10 & ICD9_detail data
	if "`input_data_name'" == "ICD_cod" {
			insheet using "FILEPATH", comma names clear
			foreach codebook_name in ICD9_detail ICD10 {
				levelsof source if list == "`codebook_name'" & upload == 1, local(`codebook_name'_sources) c
				foreach input_source in `codebook_name' ``codebook_name'_sources' { //''
					capture confirm file "FILEPATH"
					if _rc {
						display "Submitting job for `input_source'"
						!/usr/local/bin/SGE/bin/lx24-amd64/qsub -pe multi_slot 10 -l mem_free=20g -P "ADDRESS" -N "RDP_prep_`input_source'" "FILEPATH" "FILEPATH" "`input_data_name' `input_source' `codebook_name' `username'"
					}
				}
			}
		...
		// Append results together
			...
			foreach codebook_name in ICD9_detail ICD10 {
				foreach input_source in `codebook_name' ``codebook_name'_sources' { //''
					display "Checking for job for `input_source'"
					display "FILEPATH"
	...
	// Merge on location info
		...
		// Merge
			merge m:1 ihme_loc_id using `location_hierarchy', keep(1 3) assert(2 3) nogen
	// Save
		compress
		save "`FILEPATH", replace
		save "`FILEPATH", replace
	...
	// Get person-years
		keep ihme_loc_id country year
		duplicates drop
		split ihme_loc_id, p("_")
		rename ihme_loc_id1 iso3
		keep iso3 country year
		duplicates drop
		preserve
			do "FILEPATH"
			keep if location_id == .
			collapse (sum) pop, by(iso3 year) fast
			tempfile pop
			save `pop', replace
		restore
		merge 1:1 iso3 year using `pop', assert(2 3) keep(3) nogen
		collapse (sum) pop, by(country) fast
		rename pop person_years
		save "FILEPATH", replace
		save "FILEPATH", replace
