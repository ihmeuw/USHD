	...
	if `03_rescale_proportions' == 1 {
		...
		foreach package of local regression_packages {
			foreach scheme of local schemes {
				...
				capture confirm file "FILEPATH"
				if _rc {
					!/usr/local/bin/SGE/bin/lx24-amd64/qsub  -P PROJECT -pe multi_slot 5 -l mem_free=10g -N "RDP_rescale_`package'_`scheme'" "FILEPATH" "FILEPATH" "`username' `scheme' `package' `input_data_name' `today'"
				}
