...
class GarbageRedistributor(CodProcess):
    ...
    def get_computed_dataframe(self, df, cause_map):
        ...
        # remove population_group_id to create proportions at the state level
        proportion_ids = list(filter(lambda x: x not in ['population_group_id'], 
                                schema.filter({"rd_proportional": True})))
