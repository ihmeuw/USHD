...
from cod_prep.utils import report_if_merge_fail, print_log_message, CodSchema
...
class LRIRedistributor(CodProcess):
    ...
    def __init__(self, df, cause_meta_df, age_meta_df, location_meta_df):
        ...
        self.tb_other_id, start, end = tb_other_meta.loc[:, ['cause_id', 'yll_age_start', 'yll_age_end']].values[0]

        self.lri_ages = (
            age_meta_df.query(
                "age_group_days_start >= 28 & age_group_years_end <= 15"
            )
            # make sure the we aren't distributing onto disallowed ages for tb_other
            .loc[lambda d: (d['simple_age'] >= start) & (d['simple_age'] <= end), 'age_group_id']
            .tolist()
        )
    ...
    def get_adjust_df(self, df):
        ...
        if not len(df) > 0:
            return df
        # read in lri/tb redistribution proportions
        prop_df = pd.read_csv(self.conf.get_resource('lri_tb_proportions'))
        prop_df = prop_df[['location_id', 'year_id', 'tb_prop']]
        # for counties: use state-level proportions by getting parent id
        prop_df = prop_df.rename(columns={'location_id': 'parent_id'})
        df = df.merge(self.location_meta_df,
                      on=['location_id'],
                      how='left')
