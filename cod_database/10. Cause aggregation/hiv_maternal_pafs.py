...
class HIVMatPAFs(CodProcess):
    ...
    def get_computed_dataframe(self, df, cause_meta_df, location_meta_df):
        ...
        final = final.groupby(CodSchema.infer_from_data(final).id_cols,
                              as_index=False).agg(
    ...
    def clean_adjusted_data(self, df):
        ...
        if len(va_vr) > 0:
            assert {741, 366} == set(va_vr.cause_id.unique())
            va_vr = va_vr.loc[va_vr['cause_id'] != 366]
            va_vr['cause_id'] = 366
        df = pd.concat([df, va_vr], ignore_index=True)
        df.drop(columns=['pct_hiv', 'pct_maternal_hiv', 'pct_maternal', 'split_maternal'], inplace=True)
        df = (
            df.groupby(
                CodSchema.infer_from_data(df).id_cols + ["sample_size"],
                as_index=False
            )[self.all_cf_cols]
            .sum()
        )
