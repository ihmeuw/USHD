...
from cod_prep.utils import (
    report_if_merge_fail, cod_timestamp, add_tbl_metadata, report_duplicates, CodSchema
)
...
def assert_is_valid_source(source):
    ...
    if len(bad_ones) > 0:
        raise AssertionError(
            "These sources are not in 03_datases on 'FILEPATH' "
            "(remember to remove underscores from source): \n"
            "{}".format(bad_ones)
        )


def insert_names(name_table, name_df, df_name_col=None, conn_def='prodcod'):
    ...
    assert name_table in list(name_tables_to_col_name.keys()), \
        "Invalid name table: {}".format(name_table)
    name_col = name_tables_to_col_name[name_table]

    assert set(name_df.columns) == {name_col}, \
...
def map_site_id(df, site_col='site', conn_def='prodcod', upload=True):
...
def map_extract_type_id(df, source, extract_type, conn_def='prodcod'):
    nid_extract_types = {}
...
def pull_or_create_extract_type(extract_type, conn_def='prodcod'):
...
def insert_extract_type(extract_type, conn_def='prodcod'):
...
def pull_extract_type_id(extract_type, conn_def='prodcod'):
...
def insert_source_id(source, conn_def='prodcod'):
...
def pull_source_id(source, conn_def='prodcod'):
...
def pull_nid_metadata(conn_def='prodcod'):
...
def report_too_much_age_detail(nid_etid_tuple, nid_etid_df):
    ...
    if data_type_id[0] not in [7, 8, 12]:
        cod_ages = get_cod_ages(under_five_detail=True)
...
def report_no_terminal_ages(nid_etid_tuple, nid_etid_df):
    ...
    has_terminal_age = (
        (nid_etid_df['age_group_years_end'] == 125)
...
def finalize_formatting(df, source, project_id, write=False, code_system_id=None, extract_type=None,
                        conn_def='prodcod', is_active=False, refresh_cache=True,
    ...
    """
    # possible other columns used in ff
    ...
    NID_META_COLS = [
        'project_id', 'nid', 'parent_nid', 'extract_type_id', 'source', 'data_type_id',
        'code_system_id', 'is_active', 'is_mort_active'
    ]
    NID_LOCATION_YEAR_COLS = [
        'project_id', 'nid', 'extract_type_id', 'location_id', 'year_id', 'representative_id'
    ]
    ...
    df = df.merge(nid_map,
                  on=['project_id', 'nid', 'parent_nid', 'extract_type_id',
    ...
    if df_na.shape[0] > 0:
        print("""New rows for the following project/NID/extract_type_id will be added
    ...
    if write:
        ...
        # write formatted csv flat files
        ...
        for nid, extract_type_id in nid_extracts:
            ...
            print("Writing project {}, nid {}, extract_type_id {}".format(
                project_id, nid, extract_type_id))
            idf = input_df.loc[
                (input_df['nid'] == nid) &
            ...
            print("Updating cod.project_launch_set for project {pid}, nid {nid}, extract_type_id {etid}".format(
                pid=project_id, nid=nid, etid=extract_type_id))

        # update cod.project_launch_set
        ...
        update_project_launch_set(
            project_id, 0, nid_etid_phases, timestamp=format_timestamp
        )
...
def update_nid_metadata_status(project_id, nid, extract_type_id,
                               is_active=None, is_mort_active=None, conn_def='prodcod'):
    ...
    is_active_status = ezfuncs.query("""
        ...
        AND extract_type_id = {}
        AND project_id = {}
        """.format(nid, extract_type_id, project_id), conn_def=conn_def)

    if is_active_status.shape[0] == 0:
        raise AssertionError("""project {}, nid {}, extract_type_id {} does not exist in
                                cod.claude_nid_metadata""".format(project_id, nid, extract_type_id))

    if is_active_status.shape[0] > 1:
        raise AssertionError("""project_id {}, nid {}, extract_type_id {} has multiple entries
    ...
    myconn = ezfuncs.get_connection('prodcod')
    ...
    query_tools.exec_query("""
        ...
        AND extract_type_id = {}
        AND project_id = {}
        """.format(is_active, is_mort_active, nid, extract_type_id, project_id), cxn=myconn, close=True)
