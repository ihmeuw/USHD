...
data_hs[, race := car::recode(race_group, "'Hispanic'=2; 'NH Black'=4; 'NH White'=5; 'NH AIAN'=6; 'NH API'=7; else=99")]
stopifnot(nrow(data_hs[race == 99]) == 0)  # there should be no observations with missing/invalid race ID
setnames(data_hs, "race_group", "race_label")
data_hs[, race_set := pop_group_sets[population_group_set_name == "omb_1977", population_group_set_id]]
setcolorder(data_hs, c("mcnty", "year", "race", "race_label", "edu_hs"))
...
# convert 1977 OMB race/ethnicity groups to standard IDs, add race set ID from ushd_shared.population_group_set table
data_ba[, race := car::recode(race_group, "'Hispanic'=2; 'NH Black'=4; 'NH White'=5; 'NH AIAN'=6; 'NH API'=7; else=99")]
stopifnot(nrow(data_ba[race == 99]) == 0)  # there should be no observations with missing/invalid race ID
setnames(data_ba, "race_group", "race_label")
data_ba[, race_set := pop_group_sets[population_group_set_name == "omb_1977", population_group_set_id]]
setcolorder(data_ba, c("mcnty", "year", "race", "race_label", "race_set", "edu_ba"))
...
prepped_cov_id_hs <- save_prepped_covariate(path = paste0(out_dir_hs, date_time_stamp, ".rds"),
                                            description = "HS+ educational attainment by race/ethnicity, 1990-2020",
                                            prev_issues = "None",
...
prepped_cov_id_ba <- save_prepped_covariate(path = paste0(out_dir_ba, date_time_stamp, ".rds"),
                                            description = "BA+ educational attainment by race/ethnicity, 1990-2020",
                                            prev_issues = "None",
                                            sources = list(c("census_education_by_race_ethn", unique(census_meta[, file])),
                                                           c("acs_education_by_race_ethn", unique(acs_meta[, file]))),
                                            ignore_unmatched = T)
