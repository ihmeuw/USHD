...
##
##              lib_loc [character] - path to location of central USHD R library install location (needed for loading mapproj library)
##              
...
if (!dir.exists("FILEPATH")) {
  dir.create("FILEPATH")  # create an rlibs folder for this R version
  install.packages("INLA", repos=c(getOption("repos"), INLA="https://inla.r-inla-download.org/R/stable"), dep=TRUE,
                   lib = "FILEPATH")  # install INLA in this new folder
}
...
impute_covariate <- function(covar,
  ...
  if (covariate_name %in% c("correct_facil_by_race_ethn", "property_value_median_by_race_ethn")) {  # decennial estimates only
    print("This covariate has all decennial Census estimates: skipping ACS-specific SE calculation")
  } else {  # derive SE estimate for ACS data
    # reference: https://www.census.gov/content/dam/Census/library/publications/2018/acs/acs_general_handbook_2018_ch07.pdf
    covar$se <- covar$moe / 1.645
    stopifnot(FALSE %in% unique(is.na(covar$se)))  # throw error if SE is all NA
  }
  ...
  if (use_assumed_sample_sizes) {
    ...
    covar[year %in% c(1990), sampling_rate := 0.2] # 1990 decennial census long-form data were taken from a 20% sample: https://www.census.gov/history/www/through_the_decades/overview/1990.html
    covar[year %in% c(2000), sampling_rate := 0.17] # 2000 decennial census long-form data were taken from a 17% sample: https://www.census.gov/history/www/through_the_decades/overview/2000.html
    ...
    if (covariate_name %in% c("poverty_by_race_ethn", "unemployed_by_race_ethn", "edu_hs_by_race_ethn", "edu_ba_by_race_ethn", "foreign_born_by_race_ethn", "college_enr_by_race_ethn",
      ...
      if (!load_saved_intermediate) {
        ...
        covar[year %in% c(1990), neff := pop * 0.2] # 1990 decennial census long-form data were taken from a 20% sample: https://www.census.gov/history/www/through_the_decades/overview/1990.html
        covar[year %in% c(2000), neff := pop * 0.17] # 2000 decennial census long-form data were taken from a 17% sample: https://www.census.gov/history/www/through_the_decades/overview/2000.html
  ...
  if (save_plots) {
    ...
    plot(model, pdf = TRUE, prefix = paste0(output_loc, "/inla_plots/"))
    ...
    for (county_loop in unique(preds$mcnty)) {
      ...
      pdf(paste0(output_loc, "/time_series/", "mcnty_", county_loop, ".pdf"), width = 16.5, height = 11)
