...
out_dir <- "FILEPATH"
temp_dir <- "FILEPATH"
years <- c(1989, 1993, 1995, 1997:2022)  # skip 1996, it's state level estimates only
nids <- data.table(year = years, nid = c(57605, 57608:57609, 57611:57620, 57623, 57626, 57629, 81297,
                                         81299, 132623, 162649, 292282:292283, 400540, 400542, 452935,
                                         476265, 505488, 526531, 547830))

# Load data ---------------------------------------------------------------------------------------
extract_data <- function(year) {
  message("extracting ", year)
  if (year <= 2011) {
    f <- paste0(in_dir, year, "/USA_SAIPE_", year, "_ALL.CSV")
    ...
    if (year %in% 2012:2013) {
      ...
      f <- list.files(paste0(in_dir, year), pattern = paste0("USA_SAIPE_", year, "_ALL_Y....M..D...CSV"), full.names = T)
      data <- fread(f, skip = 5, header = T)
      data <- data[, .(`State FIPS Code`, `County FIPS Code`, `Poverty Percent, All Ages`, `Median Household Income`)]
    } else if (year >= 2019) {
      f <- list.files(paste0(in_dir, year), pattern = paste0("USA_SAIPE_", year, "_ALL_Y....M..D...XLS"), full.names = T)
      data <- as.data.table(read_xls(f, skip = 3))
      data <- data[, .(`State FIPS Code`, `County FIPS Code`, `Poverty Percent, All Ages`, `Median Household Income`)]
    } else {
      f <- list.files(paste0(in_dir, year), pattern = paste0("USA_SAIPE_", year, "_ALL_Y....M..D...TXT"), full.names = T)
      data <- read.fwf(f, widths = c(3, 4, 9, 9, 9, 5, 5, 5, 9, 9, 9, 5, 5, 5, 9, 9, 9, 5, 5, 5, 7,
                                     7, 7, 8, 8, 8, 5, 5, 5, 46, 3, 22))
      data <- data.table(data[, c(1, 2, 6, 21)])
    }
  ...
  data <- data[!(State.FIPS == 15 & County.FIPS == 5),]  # SAIPE doesn't report for Kalawao, HI
