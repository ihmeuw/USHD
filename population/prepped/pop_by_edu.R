...
acs_microdata <- readRDS(paste0(pop_dir, "FILEPATH", unique(acs_ipums_meta_df[, file])))[year <= 2023, -"nid"]  # read in file specified in metadata
acs_meta_df <- get_covariate_metadata("acs_pop_by_edu")
acs_tabs <- readRDS(paste0(pop_dir, "FILEPATH", unique(acs_meta_df[, file])))[, -"nid"]
census_pop_meta_df <- get_covariate_metadata("census_pop_by_edu")
census_pop <- readRDS(paste0(pop_dir, "FILEPATH", unique(census_pop_meta_df[, file])))[, -c("nid", "file_path")]
census_ipums_meta_df <- get_covariate_metadata("census_ipums_pop_by_edu")
census_ipums_pop <- readRDS(paste0(pop_dir, "FILEPATH", unique(census_ipums_meta_df[, file])))[, -"nid"]
...
# check for 0s where neighboring age_groups are not 0
...
# Save output and upload to database --------------------------------------------------------------
saveRDS(interp_pop, paste0("FILEPATH", date_time_stamp, '.rds'))
