...
extract_data <- function(year) {
  ...
  cols <- c("year", "statefip", "puma", "perwt", "age", "sex", "educd")  # only read in columns of interest (otherwise reading in data will take forever and tons of memory)
  if (year <= 2017){
  ...
  # if (year == 2020) file_path <- list.files(path = paste0(in_dir, year_range), pattern = "EDUCATION_ATTAINMENT", full.names = T)
