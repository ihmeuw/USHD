// see fit_mod_model55.R for model description
...
template<class Type>
  Type objective_function<Type>::operator() () {
    ...
    DATA_IVECTOR(U);    // under-20 indicator
    ...
    DATA_IVECTOR(U_k);    // under-20 indicator
    DATA_IVECTOR(D_k);    // data source indicator
    ...
    // RE5: data source-level random intercept (IID)
    ...
    PARALLEL_REGION nll += SCALE(GMRF(Sigma_Sparse_d), sigma_5)(re5);
    ...
    if (family == "b") {
      ...
      for(size_t i = 0; i < Y.size(); i++) {
        if (U[i] == 0) {
          logit_p[i] += re2[J[i]] + re3(T[i], A[i], R[i], E[i], M[i]) + re5(D[i]);
        } else {
          logit_p[i] += re2[J[i]] + re5(D[i]);
        }
    ...
    for(size_t k = 0; k < Y_k.size(); k++) {
        logit_p_k[k] += re2[J_k[k]] + re5(D_k[k]);
      if (U_k[k] == 1) {
        logit_p_k[k] += re3(T_k[k], A_k[k], R_k[k], E_k[k], M_k[k]);
      }
      for (size_t tp = 0; tp < s_year.cols(); tp++) {
        for (size_t ap = 0; ap < s_age.cols(); ap++) {
