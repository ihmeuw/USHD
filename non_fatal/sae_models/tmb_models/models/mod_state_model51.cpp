// see fit_mod_state_model51.R for model description
...
Type objective_function<Type>::operator() () {
  ...
  DATA_IVECTOR(U4_k);    // ages_0_to_19 indicator
  DATA_IVECTOR(U5_k);    //
  ...
  DATA_MATRIX(X4_k);    // ages_0_to_19 restricted covariates that vary by age (random slope)
  DATA_MATRIX(X5_k);
  ...
  PARAMETER_VECTOR(B4); // covars_subpop_a (ages_0_to_19)
  PARAMETER_VECTOR(B5);
  ...
  // 15 plus age-varying covariates
  PARAMETER_MATRIX(covar_subpop_re_matrix_15plus);
  PARAMETER_VECTOR(covar_subpop_log_sigmas_15plus);
  DATA_STRUCT(covar_subpop_hyperpriors_parameters_15plus, prior_type_sigma); // all have same hyperpriors
  ...
  // Random effect on age specific covariates
  for(size_t i = 0; i < covar_subpop_re_matrix_15plus.cols(); i++) {
    Type temp_sigma_15plus = exp(covar_subpop_log_sigmas_15plus[i]);
    PARALLEL_REGION nll -= dnorm(vector<Type>(covar_subpop_re_matrix_15plus.col(i)), Type(0), temp_sigma_15plus, true).sum();
  }
  ...
  for(size_t i = 0; i < covar_subpop_re_matrix_15plus.cols(); i++) {
    // loop over entries in covar_subpop_hyperpriors_parameters for covars_subpop_5plus, evaluate, and subtract that from nll
    // for now, all hyperpriors same for all covariates
    Type temp_pen_15plus = eval_prior_sigma(covar_subpop_hyperpriors_parameters_15plus, covar_subpop_log_sigmas_15plus[i]);
    REPORT(temp_pen_15plus);
    PARALLEL_REGION nll -= temp_pen_15plus;
  }
  ...
  if (family == "g") {
    ...
    for(size_t k = 0; k < Yx_k.size(); k++) {
      ...
      // add in the 15+ covariates w/ random effects
      for (size_t f = 0; f < covar_subpop_re_matrix_15plus.cols(); f++) {
        if(U5_k[k] == 0) {
          logit_m_k[k] += (covar_subpop_re_matrix_15plus(A_k[k], f) + B5[f]) * X5_k(k, f);
        }
      } // end 
