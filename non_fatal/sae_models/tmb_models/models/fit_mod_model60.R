...
##              logit(p_{j,a,t,r,e,m}) = B0 + u1_{j,t',a',r}*S(a')*S(t') + u2_{j} + u3_{t,a,r,e,m}*(U=0) + u5_{d}
...
##              u5_{d} ~ IID(sigma_5)
##              where j = area, a = age group, t = year, r = race, S(a') = age spline, S(t') = age spline, e = education, m = marital status, U = under-20 indicator (0 = ages 20+, 1 = under age 20), d = data source
...
if (nrow(dt_agg_rows) == 0) {
  dt_agg_rows <- data.table(cases = numeric(), sample_size = numeric(), area = numeric(), age = numeric(), year = numeric(), race = numeric(), edu = numeric(), marital = numeric(), post_stratify = numeric(), source_index = numeric(), agg_id = numeric(), agg_wt = numeric(), int = numeric(), covars = numeric(), covars_as = numeric(), covars_re = numeric())
}
...
tmb_data <- list(
  ...
  D_k = dt_agg_rows$source_index,
  dt_agg_rows_k = dt_agg_rows$agg_id,
  agg_wt_xwalk = dt_agg_rows$agg_wt,
  
  X = as.matrix(dt_mcnty[, c("int", covars, covars_as, covars_re), with = FALSE]),
  X_k = as.matrix(dt_agg_rows[, c("int", covars, covars_as, covars_re), with = FALSE]),
...
if (fit_model_for_starting_values & initial_fit_for_starting_values == "FALSE") {
  ...
  tmb_par <- list(
    B = rep(0, length(covars) + length(covars_as) + length(covars_re) + 1), # intercept & covariate effects
    ...
    re5 = rep(0, num_d), # data source-level random intercept
...
cat("\n\n***** Make objective function\n"); flush.console()
obj <- MakeADFun(tmb_data, tmb_par, random = c("B", paste0("re", c(1:3,5))), DLL = paste0("mod_", model), map = map)
...
fwrite(rbind(se_time, opt_time), file = paste0(output_dir, "/model_fit_time_", sex, ".rds"))
sink()