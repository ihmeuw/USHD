...
tmb_data <- list(
  ...
  X6_k = as.matrix(dt_agg[, covars_subpop_1plus, with = FALSE]),
  ...
  covar_subpop_hyperpriors_parameters_1plus = gen_covar_subpop_hyperpriors_parameters(prior_type, covar_subpop_hyperpriors_settings),
...
if (fit_model_for_starting_values & !initial_fit_for_starting_values) {
  ...
  tmb_par <- list(
    ...
    B6 = model_fit$par.fixed[which(names(model_fit$par.fixed) == "B6")], # covariate fixed
    ...
    covar_subpop_re_matrix_1plus = array(model_fit$par.random[which(names(model_fit$par.random) == "covar_subpop_re_matrix_1plus")], dim = c(num_a, length(covars_subpop_1plus))),
    covar_subpop_log_sigmas_1plus = model_fit$par.fixed[which(names(model_fit$par.fixed) == "covar_subpop_log_sigmas_1plus")]
  )
} else {
  tmb_par <- list(
    ...
    B6 = rep(0, length(covars_subpop_1plus)),
    ...
    covar_subpop_re_matrix_1plus = matrix(0, nrow = num_a, ncol = length(covars_subpop_1plus)),
    covar_subpop_log_sigmas_1plus = gen_covar_subpop_log_sigmas(covar_subpop_hyperpriors_settings, covars_subpop_1plus)
  )
}
...
obj <- MakeADFun(tmb_data, tmb_par, random = c("re1", 
                                              ...
                                              "covar_subpop_re_matrix_5plus", "covar_subpop_re_matrix_1plus"), 
