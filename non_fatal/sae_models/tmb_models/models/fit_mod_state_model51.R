...
##              logit(R_{j,a,t,r}) = B0 + (B1 + uX3_{a})*X1{j,t} + (B# + uX#_{a}})*X#{j,t}*U#=0 + 
##                                  + u1_{t,a}
##              u1_{t,a} ~ LCAR:LCAR(rho_t, rho_a, sigma)
##
...
output_dir_init <- paste0(output_dir, "/init")
dir.create(output_dir_init)
...
if (by_sex) {
  num_s <- length(get("sex", .GlobalEnv))
} else {
  num_s <- nrow(recode$sex)
}
num_j <- nrow(recode$area)
num_t <- nrow(recode$year)
num_a <- nrow(recode$age)
num_r <- nrow(recode$race)
num_k <- nrow(recode$state)
...
tmb_data <- list(
  ...
  X5_k = as.matrix(dt_agg[, covars_subpop_15plus, with = FALSE]),
  ...
  covar_subpop_hyperpriors_parameters_15plus = gen_covar_subpop_hyperpriors_parameters(prior_type, covar_subpop_hyperpriors_settings),
...
if (fit_model_for_starting_values & initial_fit_for_starting_values == "FALSE") {
  ...
  tmb_par <- list(
    ...
    B5 = model_fit$par.fixed[which(names(model_fit$par.fixed) == "B5")], # covariate fixed
    ...
    covar_subpop_re_matrix_15plus = array(model_fit$par.random[which(names(model_fit$par.random) == "covar_subpop_re_matrix_15plus")], dim = c(num_a, length(covars_subpop_15plus))),
    covar_subpop_log_sigmas_15plus = model_fit$par.fixed[which(names(model_fit$par.fixed) == "covar_subpop_log_sigmas_15plus")] 
   )
} else {
  tmb_par <- list(
    ...
    B5 = rep(0, length(covars_subpop_15plus)),
    ...
    covar_subpop_re_matrix_15plus = matrix(0, nrow = num_a, ncol = length(covars_subpop_15plus)),
    covar_subpop_log_sigmas_15plus = gen_covar_subpop_log_sigmas(covar_subpop_hyperpriors_settings, covars_subpop_15plus)
  )
}
...
obj <- MakeADFun(tmb_data, tmb_par, random = c("re1",
                                              ...
                                              "covar_subpop_re_matrix_5plus", "covar_subpop_re_matrix_15plus"), 
