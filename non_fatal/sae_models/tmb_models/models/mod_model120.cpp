// see fit_mod_model120.R for model description
...
Type objective_function<Type>::operator() () {
  ...
  DATA_IVECTOR(DR);    // data source-race indicator
  DATA_IVECTOR(W);    // data source indicator
  ...
  DATA_IVECTOR(Gallup);    // Gallup indicator
  ...
  DATA_IVECTOR(DR_k);    // data source indicator
  DATA_IVECTOR(W_k);    // data source indicator
  ...
  DATA_IVECTOR(Gallup_k);    // Gallup indicator
  ...
  // RE3: age-race-edu-marital-level random intercept (LCAR:LCAR:IID)
  PARAMETER_ARRAY(re3);
  ...
  // RE5: race-age-source-level random intercept
  PARAMETER_ARRAY(re5);
  PARAMETER(re5_log_sigma);
  Type sigma_5 = exp(re5_log_sigma);
  PARAMETER(logit_rho_5a);
  Type rho_5a = invlogit(logit_rho_5a);
  DATA_STRUCT(re5_prior_param, prior_type_sigma);
  
  // RE6: year-level random intercept (LCAR)
  PARAMETER_VECTOR(re6);
  PARAMETER(re6_log_sigma);
  Type sigma_6 = exp(re6_log_sigma);
  PARAMETER(logit_rho_6t);
  Type rho_6t = invlogit(logit_rho_6t);
  DATA_STRUCT(re6_prior_param, prior_type_sigma);

  // RE7: age-level random intercept (LCAR)
  PARAMETER_VECTOR(re7);
  PARAMETER(re7_log_sigma);
  Type sigma_7 = exp(re7_log_sigma);
  PARAMETER(logit_rho_7a);
  Type rho_7a = invlogit(logit_rho_7a);
  DATA_STRUCT(re7_prior_param, prior_type_sigma);

  // RE8: race-level random intercept (IID)
  PARAMETER_VECTOR(re8);
  PARAMETER(re8_log_sigma);
  Type sigma_8 = exp(re8_log_sigma);
  DATA_STRUCT(re8_prior_param, prior_type_sigma);
  
  // RE9: year-race-edu-marital-level random intercept (LCAR:LCAR:IID)
  PARAMETER_ARRAY(re9);
  PARAMETER(re9_log_sigma);
  Type sigma_9 = exp(re9_log_sigma);
  PARAMETER(logit_rho_9t);
  Type rho_9t = invlogit(logit_rho_9t);
  PARAMETER(logit_rho_9e);
  Type rho_9e = invlogit(logit_rho_9e);
  DATA_STRUCT(re9_prior_param, prior_type_sigma);
  
  // RE10: state-race-edu-marital-level random intercept (IID:IID:LCAR:IID)
  PARAMETER_ARRAY(re10);
  PARAMETER(re10_log_sigma);
  Type sigma_10 = exp(re10_log_sigma);
  PARAMETER(logit_rho_10e);
  Type rho_10e = invlogit(logit_rho_10e);
  DATA_STRUCT(re10_prior_param, prior_type_sigma);
  ...
  // identity matrix for source-race 1D vector
  matrix<Type> Sigma5(num_dr, num_dr); // cannot use sigma1.size() because sigma 1 here only has one dimension
  Sigma5.setIdentity();
  Eigen::SparseMatrix<Type> Sigma5_Sparse=asSparseMatrix(Sigma5);
  REPORT(Sigma5_Sparse);
  
  // RE5: race-age-data random intercept
  SparseMatrix<Type> K_5a = lcar_strmat(graph_a, rho_5a);
  // PARALLEL_REGION nll += SCALE(GMRF(Sigma5_Sparse), sigma_5)(re5);
  PARALLEL_REGION nll += SCALE(SEPARABLE(GMRF(K_5a), GMRF(Sigma5_Sparse)), sigma_5)(re5);
  
  // RE6: year-level random intercept
  SparseMatrix<Type> K_6t = lcar_strmat(graph_t, rho_6t);
  
  PARALLEL_REGION nll += SCALE(GMRF(K_6t), sigma_6)(re6);
  
  // RE7: age-level random intercept
  SparseMatrix<Type> K_7a = lcar_strmat(graph_a, rho_7a);
  
  PARALLEL_REGION nll += SCALE(GMRF(K_7a), sigma_7)(re7);
  
  // RE8: race-level random intercept
  
  PARALLEL_REGION nll += SCALE(GMRF(Sigma1_Sparse), sigma_8)(re8);
  
  // RE9: year-race-edu-marital-level random intercept
  SparseMatrix<Type> K_9t = lcar_strmat(graph_t, rho_9t);
  SparseMatrix<Type> K_9e = lcar_strmat(graph_e, rho_9e);
  
  PARALLEL_REGION nll += SCALE(SEPARABLE(GMRF(Sigma_Sparse_m), SEPARABLE(GMRF(K_9e), SEPARABLE(GMRF(Sigma1_Sparse), GMRF(K_9t)))), sigma_9)(re9);
  
  // RE10: state-race-edu-marital-level random intercept
  SparseMatrix<Type> K_10e = lcar_strmat(graph_e, rho_10e);
  
  // identity matrix for state
  matrix<Type> Sigma_w_10(num_w, num_w); // cannot use sigma1.size() because sigma 1 here only has one dimension
  Sigma_w_10.setIdentity();
  Eigen::SparseMatrix<Type> Sigma_w_10_Sparse=asSparseMatrix(Sigma_w_10);
  REPORT(Sigma_w_10_Sparse);
  
  // identity matrix for race
  matrix<Type> Sigma1_10(num_r, num_r); // cannot use sigma1.size() because sigma 1 here only has one dimension
  Sigma1_10.setIdentity();
  Eigen::SparseMatrix<Type> Sigma1_10_Sparse=asSparseMatrix(Sigma1_10);
  REPORT(Sigma1_10_Sparse);
  
  // Create sparse identity matrix for IID marital status effects
  matrix<Type> Sigma_m_10(num_m, num_m);
  Sigma_m_10.setIdentity();
  Eigen::SparseMatrix<Type> Sigma_Sparse_m_10 = asSparseMatrix(Sigma_m_10);
  REPORT(Sigma_Sparse_m_10);
  
  PARALLEL_REGION nll += SCALE(SEPARABLE(GMRF(Sigma_Sparse_m_10), SEPARABLE(GMRF(K_10e), SEPARABLE(GMRF(Sigma1_10_Sparse), GMRF(Sigma_w_10_Sparse)))), sigma_10)(re10);
  ...
  Type pen9 = eval_prior_sigma(re9_prior_param, re9_log_sigma);
  Type pen10 = eval_prior_sigma(re10_prior_param, re10_log_sigma);
  ...
  PARALLEL_REGION nll -= pen9;
  PARALLEL_REGION nll -= pen10;
  ...
  PARALLEL_REGION nll -= dnorm(logit_rho_5a, Type(0), Type(1.5), true);
  PARALLEL_REGION nll -= dnorm(logit_rho_6t, Type(0), Type(1.5), true);
  PARALLEL_REGION nll -= dnorm(logit_rho_7a, Type(0), Type(1.5), true);
  PARALLEL_REGION nll -= dnorm(logit_rho_9t, Type(0), Type(1.5), true);
  PARALLEL_REGION nll -= dnorm(logit_rho_9e, Type(0), Type(1.5), true);
  PARALLEL_REGION nll -= dnorm(logit_rho_10e, Type(0), Type(1.5), true);
  ...
  if (family == "b") {
    ...
    for(size_t i = 0; i < Y.size(); i++) {
      if (U[i] == 1) {
        logit_p[i] += re2[J[i]] + re3(A[i], R[i], E[i], M[i]) + re6(T[i]) + re7(A[i]) + re8(R[i]) + re9(T[i], R[i], E[i], M[i]) + re10(W[i], R[i], E[i], M[i]);
      } else {
        logit_p[i] += re2[J[i]] + re6(T[i]) + re7(A[i]) + re8(R[i]);
      }
      
      if (GOLD_STANDARD[i] == 0) {
        logit_p[i] += re5(DR[i], A[i]); 
      }
    ...
    for (size_t k = 0; k < Y_k.size(); k++) {
      if (U_k[k] == 1) {
        logit_p_k[k] += re2[J_k[k]] + re3(A_k[k], R_k[k], E_k[k], M_k[k]) + re6(T_k[k]) + re7(A_k[k]) + re8(R_k[k]) + re9(T_k[k], R_k[k], E_k[k], M_k[k]) + re10(W_k[k], R_k[k], E_k[k], M_k[k]);
      } else {
        logit_p_k[k] += re2[J_k[k]] + re6(T_k[k]) + re7(A_k[k]) + re8(R_k[k]);
      }
      
      if (GOLD_STANDARD_k[k] == 0) {
        logit_p_k[k] += re5(DR[k], A_k[k]); 
      }
      ...
      logit_p_k[k] += B2_Gallup * Gallup_k[k];
