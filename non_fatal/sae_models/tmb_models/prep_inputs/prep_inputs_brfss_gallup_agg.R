####################################################################################################
## Description: Prep inputs for small area models for prevalence models.
##              Specifically, combine with covariates data. Recode age groups and years to run 
##              sequentially from zero. Set up spatial disaggregation weights.
##              Also, prep CAR structure matrices for spatial, temporal, and 
##              age effects.
##
...
pacman::p_load(R.utils, data.table, TMB, car, splines, stringr, survival, Hmisc, mgcv)
...
if (length(covars) > 0) {
  ...
  cov <- data.table()
  cov_dataset_ids <- data.table()
  for (current_cov in covars) {
    print(current_cov)
    cov_data <- get_covariate_data(covariate_name = current_cov)
    # Get the covariate_dataset_id. Since we only the covariate name, the DB
    # will automatically pull the most recent dataset.
    cov_version_tmp <- get_covariate_version(covariates = current_cov)[is_best == 1]
    cov_dataset_ids <- rbind(cov_dataset_ids, cov_version_tmp)
    ...
    if (current_cov == "pop_density") {
      ...
      if (nrow(cov) == 0) {
        ...
        cov <- merge(cov, cov_data, by = c("year", "mcnty", "state", "sex", "race", "age"), all = TRUE)
      }
    }
  }
...
if (!is.null(gold_standard_source) && # if gold standard source, only create source_race_recode for not gold-standard sources
    gold_standard_source %in% unique(combined_dt$source)) {   # check if the gold standard source is in the source column (not always the case, if the gold-standard is based on the source_v2 column).
...
# Create a second version of the source variable and the source-race index.
#   Treat the landline-only version of BRFSS as a separate source from the combined landline/cellphone version.
#   The landline-only version of BRFSS is BRFSS data from year < 2011. We use the combined landline/cellphone version
#   for years >= 2011.
combined_dt[, source_v2 := fcase(source != "BRFSS", source, 
                               year >= 2011, "BRFSS_LLCP",
                               year < 2011, "BRFSS_LANDLINE_ONLY")]
if(any(is.na(combined_dt$source_v2))) stop("source_v2 has NAs -- some source/versions may not be correctly specified")
if(!is.null(gold_standard_source) && 
  gold_standard_source %in% unique(combined_dt$source_v2)){ # check whether the gold standard source is in the source_v2 column (won't be the case, if the gold-standard is based on the normal source column)
  
  # create index
  combined_dt[source_v2 != gold_standard_source, source_v2_race_recode := as.integer(.GRP) - 1L, .(source_v2, race)]
  # set up the recode map
  tmp <- na.omit(unique(combined_dt[, .(source_v2_race_recode, source_v2, race)]))
  if(nrow(tmp) > 0){
    tmp <- tmp[, .(var = "source_v2_race", source_v2_race = paste0(source_v2, "_", race), source_v2_race_recode = source_v2_race_recode, source_v2, race), by = 1:nrow(tmp)]
    recodes$source_v2_race <- tmp
    combined_dt[, c("source_v2_race", "source_v2_race_recode") := list(source_v2_race_recode, NULL)]
  }

  # create a second version of the source_v2 column that excludes the gold standard source
  # and is indexed to zero for the other sources. This is useful for the source*age
  # random effect in model 154, etc.
  combined_dt[source_v2 != gold_standard_source, source_v2_no_gs := source_v2]
  combined_dt[source_v2 == gold_standard_source, source_v2_no_gs := NA]
  source_v2_levels <- unique(combined_dt$source_v2_no_gs)
  combined_dt[, source_v2_no_gs_recode := as.integer(factor(source_v2_no_gs, levels = source_v2_levels)) - 1L]
  tmp <- na.omit(unique(combined_dt[, .(source_v2_no_gs_recode, source_v2_no_gs)]))
  recodes$source_v2_no_gs <- tmp
  combined_dt[, c("source_v2_no_gs", "source_v2_no_gs_recode") := list(source_v2_no_gs_recode, NULL)]

} else{
  # create the source_v2_race_recode, but don't subset to gold standard
  combined_dt[, source_v2_race_recode := as.integer(.GRP) - 1L, .(source_v2, race)]
  # set up the recode map
  tmp <- na.omit(unique(combined_dt[, .(source_v2_race_recode, source_v2, race)]))
  tmp <- tmp[, .(var = "source_v2_race", source_v2_race = paste0(source_v2, "_", race), source_v2_race_recode = source_v2_race_recode, source_v2, race), by = 1:nrow(tmp)]
  recodes$source_v2_race <- tmp
...
# Source v2 
# Note: in order to add a fixed effect on non-standard source, the gold-standard source has to be indexed first
# in the source_v2 column.
# Define the source_v2_levels, and make sure that the gold-standard source is ordered first

source_v2_levels <- unique(combined_dt$source_v2)
# reorder so that gold-standard source is first
if(!is.null(gold_standard_source)) source_v2_levels <- c(gold_standard_source, source_v2_levels[source_v2_levels != gold_standard_source])
combined_dt[, source_v2_index_recode := as.integer(factor(source_v2, levels = source_v2_levels)) - 1L]
if(any(is.na(combined_dt$source_v2_index_recode))) stop("NAs in source_v2_index_recode. Check that source_v2_levels is correct.")
recodes$source_v2 <- data.table(cbind.data.frame(var = "source_v2", "source_v2_index" = source_v2_levels, "source_v2_index_recode" = as.integer(factor(source_v2_levels, levels = source_v2_levels)) - 1L))
# double check that the index for the gold standard source is 0 (only if 
#   gold-standard source is in source_v2_levels at all, which isn't the case if 
#   a GS is not specified or the GS is BRFSS not BRFSS_LLCP, etc.)
if(any(source_v2_levels == gold_standard_source)) stopifnot(recodes$source_v2[source_v2_index == gold_standard_source, source_v2_index_recode] == 0L)
combined_dt[, c("source_v2_index", "source_v2_index_recode") := list(source_v2_index_recode, NULL)]

# For other random effects where we want to exclude the gold-standard, create an additional
# variable source_v2_no_gs that excludes the gold-standard source from the source_v2 variable.
# For rows where source_v2 is the gold-standard source, source_v2_no_gs will be NA.
if(!is.null(gold_standard_source)){
  source_v2_levels
  combined_dt[, source_v2_index_recode := as.integer(factor(source_v2, levels = source_v2_levels)) - 1L]
}
...
# lcar_time_neighborhood is the number of years on either side of t that are considered
#  "neighbors" of t. For example, if lcar_time_neighborhood = 1, then t-1, t, and t+1
#  are neighbors of t. If lcar_time_neighborhood = 2, then t-2, t-1, t, t+1, and t+2
#  are neighbors of t.
# By default, lcar_time_neighborhood = 1. Can change in the settings.csv file
# Note that I haven't implemented this for the LCAR effect on time/age splines (doesn't
#   seem necessary b/c already pooling many years)
if(!exists("lcar_time_neighborhood")){
  lcar_time_neighborhood <- 1
} 

graph_t <- matrix(rep(0, num_t^2), nrow = num_t, ncol = num_t)
# updated to use lcar_time_neighborhood. Previously, line was
#   graph_t[abs(row(graph_t) - col(graph_t)) == 1] <- 1 
#   With this change, we are set the diagonal to one. In the previous code, this didn't
#   happen (because row(graph_t) - col(graph_t) == 0  on the diagonal, which is not 1).
#   This doesn't matter because diagonal is subtracted out on the next line anyway.
graph_t[abs(row(graph_t) - col(graph_t)) <= lcar_time_neighborhood] <- 1 
graph_t <- diag(apply(graph_t, 1, sum)) - graph_t
graph_t <- as(graph_t, "dgTMatrix")

# lcar_age_neighborhood is the number of years on either side of a that are considered
# "neighbors" of a. Defined like lcar_age_neighborhood.
if(!exists("lcar_age_neighborhood")){
  lcar_age_neighborhood <- 1
}

if (length(ages) > 1) {
  graph_a <- matrix(rep(0, num_a^2), nrow = num_a, ncol = num_a)
  graph_a[abs(row(graph_a) - col(graph_a)) <= lcar_age_neighborhood] <- 1
...
# Create natural splines on age and time
# using age_knots[2:(length(age_knots) - 1)] and intercept = TRUE 
  # appears to give the same bases as using age_knots[1:(length(age_knots) - 1)] 
  # and intercept = FALSE.
# Note that we do NOT want to include an intercept if the spline bases are used as
#  a main effect because the intercept will be duplicative of the global intercept,
#  but we do want to include an intercept if we interact the spline with other
#  parameters

# There are multiple approaches to constructing natural spline bases. 

# set the default method for calculating natural spline bases to ns(). other methods 
# are Hmisc::rcspline.eval (related to rms::rcs())
# and mgcv::smooth.construct2(..., bs = "cr")
if(!exists("ns_method")){
  ns_method <- "ns"
  print("using ns() to calculate natural spline bases")
}

if(!exists("year_knots_num_ns")){
  # optional parameter to specify the number of knots for the natural spline, if we want
  # a different number of knots than for the linear spline
  year_knots_num_ns <- year_knots_num
}

# define the placement of the year knots for the natural spline based on year_knots_num_ns
year_knots_ns <- seq(1, length(years), length.out = year_knots_num_ns) - 1

if(ns_method == "ns"){
  # Create the age spline
  s_age_ns_intercept <- as.matrix(as.data.table(ns((1:num_a) - 1, knots = age_knots[1:(length(age_knots) - 1)], intercept = FALSE)))
  s_age_ns_no_intercept <- as.matrix(as.data.table(ns((1:num_a) - 1, knots = age_knots[2:(length(age_knots) - 1)], intercept = FALSE)))

  # Create the time spline
  s_year_ns_intercept <- as.matrix(as.data.table(ns((1:num_t) - 1, knots = year_knots_ns[1:(length(year_knots_ns) - 1)], intercept = FALSE)))
  s_year_ns_no_intercept <- as.matrix(as.data.table(ns((1:num_t) - 1, knots = year_knots_ns[2:(length(year_knots_ns) - 1)], intercept = FALSE)))
} else if(ns_method == "rcs"){
  # Create the age spline
  s_age_ns_no_intercept <- as.matrix(as.data.table(
    Hmisc::rcspline.eval(x = 1:num_a - 1, knots = age_knots, inclx = TRUE)
  ))
  s_age_ns_intercept <- NULL

  # create the year spline
  s_year_ns_no_intercept <- as.matrix(as.data.table(
    Hmisc::rcspline.eval(x = 1:num_t - 1, knots = year_knots_ns, inclx = TRUE)
  ))
  s_year_ns_intercept <- NULL

  print("using Hmisc/rms to construct natural spline bases")

} else if (ns_method == "mgcv_cr"){
  # construct the Cardinal spline bases
  # NOTE -- I think it would actually make sense to use an LCAR effect if we do the
  #   cardinal bases with an intercept b/c the value at the params corresponds to the spline value at knot
  # https://www.maths.ed.ac.uk/~swood34/gsm.pdf
  # https://stat.ethz.ch/R-manual/R-devel/library/mgcv/html/smooth.construct.cr.smooth.spec.html

  # Create the age spline
  s_age_ns_intercept <- as.matrix(as.data.table(
    mgcv::smooth.construct2(
      object = s(age, k = length(age_knots), bs = "cr"), 
      data = list(age = (1:num_a)-1), 
      knots = list(age = age_knots))$X
  ))
  s_age_ns_no_intercept <- s_age_ns_intercept #NULL # could construct by fitting gam() on dummy data and predicting the "lpmatrix", but not sure this makes sense
  warning("temp copying s_age_ns_intercept to s_age_ns_no_intercept -- need to see if there is a reasonable way to specify the bases with this method without intercept")
  # Create the time spline
  
  s_year_ns_intercept <- as.matrix(as.data.table(
    mgcv::smooth.construct2(
      object = s(year, k = length(year_knots_ns), bs = "cr"), 
      data = list(year = (1:num_t)-1), 
      knots = list(year = year_knots_ns))$X
  ))
  s_year_ns_no_intercept <- s_year_ns_intercept # NULL # could construct by fitting gam() on dummy data and predicting the "lpmatrix", but not sure this makes sense
  stop("temp copying s_year_ns_intercept to s_year_ns_no_intercept -- need to see if there is a reasonable way to specify the bases with this method without intercept before using in final models")
  print("using mgcv to construct natural spline bases")
} else {
  stop("ns_method must be one of 'ns', 'rcs', or 'mgcv_cr'")
}
...
###### Save prepped data
setkeyv(combined_dt, c("area", "year", "sex", "race", "age", "source", strat_vars))
save(graph_j, graph_t, graph_a, graph_e, graph_as, graph_ts, s_age, s_year, s_age_ns_intercept, s_age_ns_no_intercept, s_year_ns_intercept, s_year_ns_no_intercept, file = paste0(output_dir, "/re_graphs.RData"))
...
num_d_v2 <- nrow(recodes$source_v2)
num_dr <- nrow(recodes$source_race)
num_dr_v2 <- nrow(recodes$source_v2_race)
...
#### Save num variables
save(num_age_spline, num_year_spline, num_s, num_j, num_t, num_a, num_r, num_d, num_d_v2, num_dr, num_dr_v2, num_v, num_e, num_m, num_f, num_g, num_w, file = paste0(output_dir, "/num_vars.RData"))
...
if ("BRFSS" %in% source_levels) {
  ...
  if (nrow(dt_age_80) > 0) {
    ...
    if (!identical(c("edu", "marital"), sort(strat_vars))) {
    ...
    pop[, pop2 := sum(pop), by = c("mcnty", "state", "age", "race", "sex", strat_vars)]
    pop[, agg_pop2 := sum(pop), by = c("mcnty", "state", "race", "sex", strat_vars)]
    pop[, wt2 := pop2 / agg_pop2]
    
    # Third level of the cascade (drop year and mcnty)
    pop[, pop3 := sum(pop), by = c("state", "age", "race", "sex", strat_vars)]
    pop[, agg_pop3 := sum(pop), by = c("state", "race", "sex", strat_vars)]
    pop[, wt3 := pop3 / agg_pop3]
    
    # Fourth level of the cascade (drop year and race)
    pop[, pop4 := sum(pop), by = c("state", "sex", strat_vars, "age")]
    pop[, agg_pop4 := sum(pop), by = c("state", "sex", strat_vars)]
    pop[, wt4 := pop4 / agg_pop4]
    
    # Fifth level of the cascade (drop year, race, and sex)
    pop[, pop5 := sum(pop), by = c("state", "age", strat_vars)]
    pop[, agg_pop5 := sum(pop), by = c("state", strat_vars)]
    ...
    stopifnot(pop[, sum(agg_wt2), by = c("mcnty", "state", "year", "sex", "race", strat_vars)][, max(abs(V1 - 1))] < 1e-10)
...
if (any(c("mdcr", "max", "HCUP", "mdcr_eligibility") %in% data_file$name)) { # Clinical data, except MarketScan
  ...
  combined_dt_clinical$cases <- as.numeric(combined_dt_clinical$cases)
...
if (!is.null(cov)) {
  ...
  saveRDS(cov_dataset_ids, file = paste0(output_dir, "/cov_dataset_versions.rds"))
