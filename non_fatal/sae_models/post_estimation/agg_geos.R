...
agg_args <- readRDS(paste0(output_dir_draws_est,"/agg_args.rds"))$agg_geos
sex <- agg_args[task_id, sex]
year <- agg_args[task_id, year]
level <- agg_args[task_id, level]
race <- agg_args[task_id, race]

if (interactive()) {
  (output_dir_draws_est <- paste0(draws_est_output_dir, run_date, "/"))
  draws_file <- paste0("draws_", area_var, "_", year, "_", sex, "_", race, "_", edu, ".rds")
}
...
# So multiplying pop*wt has not effect when level is state or natl
...
agg_geo <- function(draws_file, wts, std_wt, draws_out_dir, est_out_dir, var_name = "pred") {
  ...
  if (by_source) {
    ...
    merge_vars <- c("level", "area2", "year", "sex", "race", "age", source_var, "sim")
  } else {
...
# aggregate draws
agg_geo(paste0("draws_", area_var, "_", year, "_", sex, "_", race, "_", edu, ".rds"), wts, std_wt, draws_out_dir = paste0(output_dir_draws_est, "/draws/"), est_out_dir = paste0(output_dir_draws_est, "/est/"))
