...
agg_args <- readRDS(paste0(output_dir_draws_est,"/agg_args.rds"))$agg_races
...
agg_race <- function(draws_file, wts, std_wt, draws_out_dir, est_out_dir, var_name = "pred") {
  ...
  if (by_source) {
    ...
    merge_vars <- c("level", "area", "year", "sex", "age", source_var, "sim")
  } else {
...
# aggregate draws
agg_race(paste0("draws_", area_var, "_", year, "_", sex, "_RACE_", edu, ".rds"), wts, std_wt, draws_out_dir = paste0(output_dir_draws_est, "/draws/"), est_out_dir = paste0(output_dir_draws_est, "/est/"))
