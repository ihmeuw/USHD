...
agg_args <- readRDS(paste0(output_dir_draws_est,"/agg_args.rds"))$agg_sex
...
agg_sex <- function(draws_file, wts, std_wt, draws_out_dir, est_out_dir, var_name = "pred") {
  ...
  # NOTE: have not added changed to this yet to accommodate the adjusted column
  if (length(sexes) == 1) {
    temp <- copy(draws)
    temp[, pred := 0]
  ...
  if (by_source) {
    ...
    merge_vars <- c("level","area","year","race","age", source_var, "sim")
  } else {
...
# aggregate draws
agg_sex(paste0("draws_", level, "_", year, "_SEX_", race, "_", edu, ".rds"), wts, std_wt, draws_out_dir = paste0(output_dir_draws_est, "/draws/"), est_out_dir = paste0(output_dir_draws_est, "/est/"))
