...
set_queue_dynamically <- function(queue = "QUEUE", num_jobs = 1, fthread = 1, m_mem_free = "2G", h_rt = "1-0:00:00", archive = TRUE, priority = "RAM") {
  library(stringr)
  ...
  if (runtime_days > 3) {
    message("Requested run time is greater than 3 days; must use QUEUE")
    return("QUEUE")
  }
  ...
  #### Drop QUEUE and QUEUE
  system_table <- system_table[!(PARTITION %in% c("QUEUE", "QUEUE"))]
  ...
  idle_available <- system_table[STATE == "idle" & PARTITION %in% c("QUEUE", "QUEUE", "QUEUE*") & as.integer(available_cpus) >= as.integer(fthread) & as.integer(FREE_MEM) >= as.integer(str_replace(m_mem_free, "G", "")) * 1024, list(available_cpus = sum(available_cpus), free_mem = sum(as.integer(FREE_MEM)), .N), by = PARTITION]
  ...
  ## Note that we only consider nodes whose total CPU and RAM exceed our request
  queue_available <- system_table[PARTITION %in% c("QUEUE", "QUEUE", "QUEUE*") & as.integer(available_cpus) >= as.integer(fthread) & as.integer(FREE_MEM) >= as.integer(str_replace(m_mem_free, "G", "")) * 1024, list(available_cpus = sum(available_cpus), free_mem = sum(as.integer(FREE_MEM)), .N), by = PARTITION]
  ...
  queue_totals <- system_table[PARTITION %in% c("QUEUE", "QUEUE", "QUEUE*") & as.integer(total_cpus) >= as.integer(fthread) & as.integer(MEMORY) >= as.integer(str_replace(m_mem_free, "G", "")) * 1024, list(available_cpus = sum(available_cpus), free_mem = sum(as.integer(FREE_MEM))), by = PARTITION]
  ...
  if (!is.null(return_queue) & !is.na(return_queue)) {
    ...
    message(paste0("No suitable node(s) with resource availability found; defaulting to QUEUE"))
    return("QUEUE")
  }
}