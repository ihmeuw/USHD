...
get_settings <- function(file, dest.env = .GlobalEnv) {

  # load settings
  settings <- read.csv(file, stringsAsFactors = F, header = F)
...
check_settings <- function(dir, validation = F) {
  ...
  # check that the main directory exists and is on share
  ...
  all_settings <- c("model_class", "model", "area_var", "raking_area_var",
                    "years", "ages", "sexes", "by_race", "races", "race_labels",
                    "covars", "covars_as", "covars_trans", "n.sims",
                    "adjmat_file", "deaths_file", "pop_file", "covar_file", "covar_as_file",
                    "geoagg_files", "shape_file", "age_std_file", "ref_lt_file",
                    "raking_draws_dir", "raking_est_dir")
  ...
  if (fixed_age_time & model == "4") stop("'fixed_age_time' = TRUE is not implemented for model 4")
  ...
  if (!by_race & races[1] != 9) stop("'by_race' is FALSE, but races does not equal 9")
  ...
  # check if the user wants to run with a file that specifies state-years with Multiracial/Hispanic options
  if(!is.null(subset_hisp_mr)) {
    if(subset_hisp_mr) {
      if(!("subset_hisp_mr_path" %in% race_avail)){
        stop("Cannot take out state-years without Multiracial/Hispanic options without necessary file")
      } else if(is.null(subset_hisp_mr_path)){
        stop("Cannot take out state-years without Multiracial/Hispanic options without necessary file")
      }
    }
  } else if (!is.null(subset_hisp_mr_path)) {
    stop("Path to file that removes state-years without Multiracial/Hispanic options available, but subset_hisp_mr not specified")
  } else if (is.null(subset_hisp_mr_path) & is.null(subset_hisp_mr) & by_race) {
    message("Warning: specified to run by race, but missing options for removing state-years without Multiracial/Hispanic options")
  }
    
  # check if the user wants to run with the new old age qx extension
  if(!is.null(qx_diff_extension)) {
    if(qx_diff_extension) {
      if(is.null(qx_diff_extension_coefs)){
        stop("Cannot extend qx using new method without necessary coefficients")
      }
    }
  } else if (!is.null(qx_diff_extension_coefs)) {
    stop("Path to qx extension coefficients available, but qx_diff_extension not specified")
  }
  
  # check that all input files exist
  all_files <- list("adjmat_file" = adjmat_file, "deaths_file" = deaths_file, "pop_file" = pop_file,
  ...
  if (!is.null(qx_diff_extension_coefs)) all_files[["qx_diff_extension_coefs"]] <- qx_diff_extension_coefs
  ...
  rdata_files <- c("gs_file")
  rds_files <- c("deaths_file", "pop_file", "adjmat_file", "geoagg_files", "shape_file",
                 "covar_file", "covar_as_file", "age_std_file")
  if("subset_hisp_mr_path" %in% ls(envir = .GlobalEnv)) rds_files <- c(rds_files,"subset_hisp_mr_path")
  csv_files <- c("ref_lt_file")
  
  if(!is.null(qx_diff_extension_coefs)) csv_files <- c(csv_files,"qx_diff_extension_coefs")
  for (ff in names(all_files)) {
    if (ff %in% rdata_files) type <- "rdata"
  ...
  all_vars <- c(area_var, "year", "sex", "age", if (by_race) "race", "deaths")
  ...
  all_vars <- c(area_var, "year", "sex", "age", if (by_race) "race", "pop")
  ...
  if (!is.null(covars)) {
    ...
    if (sum(!years %in% covar$year)) stop("not all years in 'years' are present in 'covar_file'")
    if (length(unique(covar[[area_var]])) != num_j) stop("there are a different number of counties in 'covar_file' than in 'adjmat_file'")
    if (sum(!unique(covar[[area_var]]) %in% 0:(num_j - 1))) stop("area variable is not coded correctly in 'covar_file' (i.e., sequentially from 0)")
    if (sum(is.na(covar[year %in% years, all_vars, with = F]))) stop("NAs in 'covar_file' for specified years")
    rm(covar)
  }
  ...
  if (!is.null(covars_as)) {
    ...
    if (sum(!years %in% covar$year)) stop("not all years in 'years' are present in 'covar_as_file'")
    if (sum(!sexes %in% covar$sex)) stop("not all sexes in 'sexes' are present in 'covar_as_file'")
    if (sum(!ages %in% covar$age)) stop("not all ages in 'ages' are present in 'covar_as_file'")
    if (length(unique(covar[[area_var]])) != num_j) stop("there are a different number of counties in 'covar_as_file' than in 'adjmat_file'")
    if (sum(!unique(covar[[area_var]]) %in% 0:(num_j - 1))) stop("area variable is not coded correctly in 'covar_as_file' (i.e., sequentially from 0)")
    if (sum(is.na(covar[year %in% years, all_vars, with = F]))) stop("NAs in 'covar_as_file' for specified years")
    rm(covar)
  }
  
  if(!is.null(covars_re) & is.null(covar_re_file)) stop("race-ethnicity covariates specified but there is not a race-ethnicity covariates file specified")
  if(!is.null(covars_re)) {
    covar <- readRDS(covar_re_file)
    if (!class(covar)[1] == "data.table") stop("object in 'covar_re_file' should be a data.table")
    all_vars <- c(area_var, "year", "race", covars_re)
    miss <- setdiff(all_vars, names(covar))
    if (length(miss)) stop(paste("Variables missing from 'covar_re_file':", paste(miss, collapse = "; ")))
    if (sum(!years %in% covar$year)) stop("not all years in 'years' are present in 'covar_re_file'")
    if (sum(!races %in% covar$race)) stop("not all race/ethnicity groups in 'races' are present in 'covar_re_file'")
    if (length(unique(covar[[area_var]])) != num_j) stop("there are a different number of counties in 'covar_re_file' than in 'adjmat_file'")
    if (sum(!unique(covar[[area_var]]) %in% 0:(num_j - 1))) stop("area variable is not coded correctly in 'covar_re_file' (i.e., sequentially from 0)")
    if (sum(is.na(covar[year %in% years, all_vars, with = F]))) stop("NAs in 'covar_re_file' for specified years")
    rm(covar)
  }
  ...
  if (length(intersect(covars, covars_re))) stop("'covars' and 'covars_re' cannot contain any of the same items")
  if (length(intersect(covars_re, covars_as))) stop("'covars_re' and 'covars_as' cannot contain any of the same items")
  ...
  if (!is.null(covars_trans)) {
    ...
    if (sum(!names(covars_trans) %in% c(covars, covars_as, covars_re))) stop("the name of each item in 'covars_trans' must also be in 'covars' or 'covars_as'")
    
    covar <- list()
    if(!is.null(covars)) {
      covar[[1]] <- readRDS(covar_file)[,(c("mcnty","year",covars)), with=F]
    }
    if(!is.null(covars_as)) {
      covar[[length(covar) + 1]] <- readRDS(covar_as_file)[,(c("mcnty","year","age","sex",covars_as)), with=F]
    }
    if(!is.null(covars_re)) {
      covar[[length(covar) + 1]] <- readRDS(covar_re_file)[,(c("mcnty","year","race",covars_re)), with=F]
    }
  ...
  # make sure that combine_mc is True when misclassification is specified because that is currently the only
  # option that works
  if(!is.null(misclassification_correction)) {
    if(misclassification_correction) {
      if(is.null(combine_mc)) stop("Combined misclassification must be specified")
      if(!combine_mc) stop("Combined misclassification must be specified")
    }
  }
  ...
  # check the format of the qx extension file
  if(!is.null(qx_diff_extension_coefs)){
    qx_coef <- fread(qx_diff_extension_coefs)
    all_vars <- c("age","sex","intercept","slope")
    miss <- setdiff(all_vars,names(qx_coef))
    if(length(miss)) stop(paste("Variables missing from 'qx_diff_extension_coefs':", paste(miss, collapse = "; ")))
    rm(qx_coef)
  }
  ...
  if (validation) {
    load(gs_file)
    if (length(setdiff(c("gs_mx", "gs_ex"), ls()))) stop("'gs_file' should contain objects named 'gs_mx' and 'gs_ex'")
