...
if (!interactive()) {
  ...
  queue <- args[5]
  sing_image <- args[6]
  initial_fit_for_starting_values <- as.logical(args[7])
  run_date_hold <- args[8]
}
...
###### Set folds to 1 for non-validation runs
if (!exists("n_folds") | type != "validation") {
  n_folds <- 1
...
if (type %in% c("models_only", "all", "validation")) {
  ...
  jids[, "prep_inputs"] <- sbatch(code = paste0(repo, model_class, "/prep_inputs/", prep_inputs_file),
                                  name = "prep_inputs",
                                  arguments = c(repo, output_dir, output_dir_draws_est, settings_loc, imp, n_folds),
                                  fthread = 1, m_mem_free = "150G", h_rt = "0-01:00:00", archive = TRUE,
                                  skip_if_exists = if (resub) paste0(output_dir, "/data.rds"),
                                  queue = auto_queue, sgeoutput = output_dir,
                                  sing_image = sing_image)
  
  Sys.sleep(30)
  
  if (fit_model_for_starting_values) {
    if (race_together) {
      if (by_sex) {
        jids[sex != 3 & (!by_race | race != 1),
             fit_mod_init := sbatch(code = paste0(repo, model_class, "/models/fit_mod_", model, ".R"),
                                    name = paste0("fit_mod_", model, "_", sex, ".", fold, "_init"), 
                                    arguments = c(repo, output_dir, settings_loc, sex, "TRUE"),
                                    hold = na.omit(unique(c(prep_inputs))),
                                    fthread = fthread_mod, m_mem_free = m_mem_free_mod, queue = auto_queue, h_rt = h_rt_mod,
                                    skip_if_exists = if (resub) paste0(output_dir, "/init/model_fit_", sex, ".rds"),
                                    archive = TRUE, intel = T, project = "PROJECT",
                                    mkl_threads = fthread_mod, omp_threads = 1, # fastest combination of threads
                                    shell = "FILEPATH",
                                    sing_image = "FILEPATH",
                                    sgeoutput = output_dir,
                                    type = "TMB"), by = c("sex", "fold")]
        Sys.sleep(30)
      } else {
        jids[sex != 3 & (!by_race | race != 1), "fit_mod_init"] <- sbatch(code = paste0(repo, model_class, "/models/fit_mod_", model, ".R"),
                                                                          name = paste0("fit_mod_", model, ".", fold, "_init"), 
                                                                          arguments = c(repo, output_dir, settings_loc, "NULL", "TRUE"),
                                                                          hold = na.omit(unique(c(jids[sex != 3 & (!by_race | race != 1)]$prep_inputs))),
                                                                          fthread = fthread_mod, m_mem_free = m_mem_free_mod, queue = auto_queue, h_rt = h_rt_mod,
                                                                          skip_if_exists = if (resub) paste0(output_dir, "/init/model_fit.rds"),
                                                                          archive = TRUE, intel = T, project = "PROJECT",
                                                                          mkl_threads = fthread_mod, omp_threads = 1, # fastest combination of threads
                                                                          shell = "FILEPATH",
                                                                          sing_image = "FILEPATH",
    ...
    if (by_sex) {
      jids[sex != 3 & (!by_race | race != 1),
           fit_mod := sbatch(code = paste0(repo, model_class, "/models/fit_mod_", model, ".R"),
                             ...
                             hold = na.omit(unique(c(prep_inputs))),
                             fthread = fthread_mod, m_mem_free = m_mem_free_mod, queue = auto_queue, h_rt = h_rt_mod,
                             skip_if_exists = if (resub) paste0(output_dir, "/model_fit_", sex, ".rds"),
                             archive = TRUE, intel = T, project = "PROJECT",
                             mkl_threads = fthread_mod, omp_threads = 1, # fastest combination of threads
                             shell = "FILEPATH",
                             sing_image = "FILEPATH",
      ...
      jids[sex != 3 & (!by_race | race != 1), "fit_mod"] <- sbatch(code = paste0(repo, model_class, "/models/fit_mod_", model, ".R"),
                                                                   ...
                                                                   arguments = c(repo, output_dir, settings_loc, initial_fit_for_starting_values, imp),
                                                                   hold = na.omit(unique(c(jids[sex != 3 & (!by_race | race != 1)]$prep_inputs))),
                                                                   fthread = fthread_mod, m_mem_free = m_mem_free_mod, queue = auto_queue, h_rt = h_rt_mod,
                                                                   skip_if_exists = if (resub) paste0(output_dir, "/model_fit_", imp, ".rds"),
                                                                   archive = TRUE, intel = T, project = "PROJECT",
                                                                   mkl_threads = fthread_mod, omp_threads = 1, # fastest combination of threads
                                                                   shell = "FILEPATH",
                                                                   sing_image = "FILEPATH",
  ...
  if (by_sex) {
    ...
    jids[sex != 3 & (!by_race | race != 1),
         pred_draws := sbatch(code = paste0(repo, model_class, "/pred_draws.R"),
                              hold = na.omit(unique(jids$fit_mod)),

                              # NOTE dummy race value
                              arguments = c(repo, output_dir, output_dir_draws_est, settings_loc, sex, races_submit, validate, resub, by_sex, draw_width),
                              name = paste0("pred_draws_", sex, "_", imp),
                              skip_if_exists = if (resub) paste0(output_dir_draws_est, "/initial_sims_", sex, "_99_", edu, ".rds"),
                              fthread = 4, m_mem_free = paste0(mem_pred, "G"),
                              h_rt = "4:00:00", archive = TRUE,
    ...
    jids[sex != 3 & (!by_race | race != 1),
         pred_draws := sbatch(code = paste0(repo, model_class, "/pred_draws.R"),
                              # NOTE dummy race value
                              arguments = c(repo, output_dir, output_dir_draws_est, settings_loc, 99, races_submit, validate, resub, by_sex, draw_width),
                              name = paste0("pred_draws_", imp),
                              hold = na.omit(unique(jids$fit_mod)),
                              skip_if_exists = if (resub) paste0(output_dir_draws_est, "/initial_sims_99_1.rds"),
                              fthread = 4, m_mem_free = paste0(mem_pred, "G"),
                              h_rt = "4:00:00", archive = TRUE,
                              project = "PROJECT", queue = auto_queue,
                              sgeoutput = output_dir, sing_image = sing_image), 
...
# submit single cause hold (hold on pred draws)
sbatch(code = paste0(repo, "/submit_single_cause_hold.R"),
         name = paste0("submit_hold_", run_date),
         hold = na.omit(unique(jids$pred_draws)),
         arguments = c(repo, output_dir, output_dir_draws_est, settings_loc, queue, sing_image, run_date, imp),
         fthread = 1, m_mem_free = "1G", queue = auto_queue, h_rt = "0-01:00:00",
         archive = TRUE, sgeoutput = output_dir)
