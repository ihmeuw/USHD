...
##
## Input:       None
## Output:      Combined microdata files for BRFSS, with requisite demographic and general health data. Data are saved to the Limited Use
##              folder in FILEPATH.
##
###########################################################################################################################################
...
pops <- get_population_data(population_name = "pop_by_race_ethn_1977", covariate_dataset_id = 167)
...
# data_combined[is.na(age_continuous) & !is.na(imp_age_continuous), age_continuous := imp_age_continuous]
# xtabs(~ age_bin + year, data_combined, addNA = TRUE)
...
#### Process gen_health
gen_health_map <- data.table("gen_health" = 1:5, "genhlth" = c("Excellent", "Very good", "Good", "Fair", "Poor"))
data_combined <- merge(data_combined, gen_health_map, by = "genhlth", all.x = TRUE)
data_combined[, genhlth := NULL]

#### Rename weights variable
setnames(data_combined, "wt", "weights")

#### Define an tooth loss variable (combining categories of 6+ teeth removed and all teeth removed)
...
## First demonstrate that no existing CBSA codes contain 999
nrow(data_combined[grepl("999", cbsa)])
...
#### Process falls variables
data_combined[!is.na(fall3mn) & !is.na(fallinj), fall_person_years := 0.25] # 3 months
data_combined[!is.na(fall12mn) & !is.na(fallinj), fall_person_years := 1] # 12 months

#### Process dementia
setnames(data_combined, "cidiagaz", "dementia")

#### Process cancer
data_combined[cncrtyp == 1, survive_breast := 1]
data_combined[cncrtyp != 1 & !is.na(cncrtyp), survive_breast := 0]
data_combined[cncrtyp == 2, survive_prostate := 1]
data_combined[cncrtyp != 2 & !is.na(cncrtyp), survive_prostate := 0]
...
data_missing_gen_health_race <- data_combined[(is.na(mcnty) & is.na(cbsa) & is.na(cbsa_mcnty_code) & is.na(state)) | is.na(year) | is.na(sex) | is.na(race) | is.na(age_bin) | is.na(gen_health) | is.na(edu_code) | is.na(marital_ushd) | is.na(weights)]
data_combined_gen_health_race <- data_combined[!(is.na(mcnty) & is.na(cbsa) & is.na(cbsa_mcnty_code) & is.na(state)) & !is.na(year) & !is.na(sex) & !is.na(race) & !is.na(age_bin) & !is.na(gen_health) & !is.na(edu_code) & !is.na(marital_ushd) & !is.na(weights)]
...
data_combined_agg_gen_health_race <- data_combined_gen_health_race[, list(gen_health_45_count = sum(gen_health_45), gen_health_45_weighted_count = weighted.mean(gen_health_45, weights) * .N, 
                                                                          weights = sum(weights), sample_size = .N), by = list(mcnty, cbsa_mcnty_code, state, year, sex, race, age_bin, edu_code, marital_ushd)]
stopifnot(nrow(data_combined_agg_gen_health_race[is.na(gen_health_45_count)]) == 0 & nrow(data_combined_agg_gen_health_race[is.na(gen_health_45_weighted_count)]) == 0)

#### Save collapsed data sets
saveRDS(data_missing_gen_health_race, file = paste0(output_dir, "/brfss_agg_missing_data_gen_health_race.rds"))
saveRDS(data_combined_agg_gen_health_race, file = paste0(output_dir, "/brfss_agg_gen_health_race.rds"))
rm(data_combined_gen_health_race, data_missing_gen_health_race, data_combined_agg_gen_health_race)
gc()
...
######## 7. Create a data set for analyzing asthma
#### Drop observations without mcnty, year, sex, age, race and/or education; save these to separate files for follow-up
...
######## 8. Collapse asthma by county-year-age-sex and either race/ethnicity or education
...
######## 9. Create a data set for analyzing COPD
#### Drop observations without mcnty, year, sex, age, race and/or education; save these to separate files for follow-up
...
######## 10. Collapse COPD by county-year-age-sex and either race/ethnicity or education
...
######## 11. Create a data set for analyzing arthritis
#### Drop observations without mcnty, year, sex, age, race and/or education; save these to separate files for follow-up
...
######## 12. Collapse arthritis by county-year-age-sex and either race/ethnicity or education
...
######## 13. Create a data set for analyzing depression
#### Drop observations without mcnty, year, sex, age, race and/or education; save these to separate files for follow-up
...
######## 14. Collapse depression by county-year-age-sex and either race/ethnicity or education
...
######## 15. Create a data set for analyzing tooth loss
#### Drop observations without mcnty, year, sex, age, race and/or education; save these to separate files for follow-up
...
######## 16. Collapse tooth loss by county-year-age-sex and either race/ethnicity or education
...
######## 19. Create a data set for analyzing anxiety
#### Process anxiety variable
#### Drop observations without mcnty, year, sex, age, race and/or education; save these to separate files for follow-up
...
######## 20. Collapse anxiety by county-year-age-sex and either race/ethnicity or education
...
######## 21. Create a data set for analyzing falls
#### Drop observations without mcnty, year, sex, age, race and/or education; save these to separate files for follow-up
data_missing_falls_race <- data_combined[(is.na(mcnty) & is.na(cbsa) & is.na(cbsa_mcnty_code) & is.na(state)) | is.na(year) | is.na(sex) | is.na(race) | is.na(age_bin) | is.na(fallinj) | is.na(fall_person_years) | is.na(edu_code) | is.na(marital_ushd) | is.na(weights)]
data_combined_falls_race <- data_combined[!(is.na(mcnty) & is.na(cbsa) & is.na(cbsa_mcnty_code) & is.na(state)) & !is.na(year) & !is.na(sex) & !is.na(race) & !is.na(age_bin) & !is.na(fallinj) & !is.na(fall_person_years) & !is.na(edu_code) & !is.na(marital_ushd) & !is.na(weights)]
if (nrow(data_missing_falls_race) + nrow(data_combined_falls_race) != nrow(data_combined)) {
  warning(paste0("The total number of rows in the missingness and complete-case data sets for race/ethnicity (", nrow(data_missing_falls_race) + nrow(data_combined_falls_race), ") does not equal the number of rows in the original data set (", nrow(data_combined), "). Please check this." ))
}


######## 22. Collapse falls by county-year-age-sex and either race/ethnicity or education
data_combined_agg_falls_race <- data_combined_falls_race[, list(falls_count = sum(fallinj), falls_weighted_count = weighted.mean(fallinj, weights) * .N, weights = sum(weights), sample_size = .N, person_years = sum(fall_person_years)), by = list(mcnty, cbsa_mcnty_code, state, year, sex, race, age_bin, edu_code, marital_ushd)]
stopifnot(nrow(data_combined_agg_falls_race[is.na(falls_count)]) == 0 & nrow(data_combined_agg_falls_race[is.na(falls_weighted_count)]) == 0)

#### Save collapsed data sets
saveRDS(data_missing_falls_race, file = paste0(output_dir, "/brfss_agg_missing_data_falls_race.rds"))
saveRDS(data_combined_agg_falls_race, file = paste0(output_dir, "/brfss_agg_falls_race.rds"))
rm(data_combined_falls_race, data_missing_falls_race, data_combined_agg_falls_race)
gc()


######## 23. Create a data set for analyzing dementia
#### Drop observations without mcnty, year, sex, age, race and/or education; save these to separate files for follow-up
data_missing_dementia_race <- data_combined[(is.na(mcnty) & is.na(cbsa) & is.na(cbsa_mcnty_code) & is.na(state)) | is.na(year) | is.na(sex) | is.na(race) | is.na(age_bin) | is.na(dementia) | is.na(edu_code) | is.na(marital_ushd) | is.na(weights)]
data_combined_dementia_race <- data_combined[!(is.na(mcnty) & is.na(cbsa) & is.na(cbsa_mcnty_code) & is.na(state)) & !is.na(year) & !is.na(sex) & !is.na(race) & !is.na(age_bin) & !is.na(dementia) & !is.na(edu_code) & !is.na(marital_ushd) & !is.na(weights)]
if (nrow(data_missing_dementia_race) + nrow(data_combined_dementia_race) != nrow(data_combined)) {
  warning(paste0("The total number of rows in the missingness and complete-case data sets for race/ethnicity (", nrow(data_missing_dementia_race) + nrow(data_combined_dementia_race), ") does not equal the number of rows in the original data set (", nrow(data_combined), "). Please check this." ))
}


######## 24. Collapse dementia by county-year-age-sex and either race/ethnicity or education
data_combined_agg_dementia_race <- data_combined_dementia_race[, list(dementia_count = sum(dementia), dementia_weighted_count = weighted.mean(dementia, weights) * .N, weights = sum(weights), sample_size = .N), by = list(mcnty, cbsa_mcnty_code, state, year, sex, race, age_bin, edu_code, marital_ushd)]
stopifnot(nrow(data_combined_agg_dementia_race[is.na(dementia_count)]) == 0 & nrow(data_combined_agg_dementia_race[is.na(dementia_weighted_count)]) == 0)

#### Save collapsed data sets
saveRDS(data_missing_dementia_race, file = paste0(output_dir, "/brfss_agg_missing_data_dementia_race.rds"))
saveRDS(data_combined_agg_dementia_race, file = paste0(output_dir, "/brfss_agg_dementia_race.rds"))
rm(data_combined_dementia_race, data_missing_dementia_race, data_combined_agg_dementia_race)
gc()


######## 25. Create a data set for analyzing prostate and breast cancer
#### Drop observations without mcnty, year, sex, age, race and/or education; save these to separate files for follow-up
data_missing_prostate_race <- data_combined[(is.na(mcnty) & is.na(cbsa) & is.na(cbsa_mcnty_code) & is.na(state)) | is.na(year) | is.na(sex) | is.na(race) | is.na(age_bin) | is.na(prostate) | is.na(edu_code) | is.na(marital_ushd) | is.na(weights)]
data_combined_prostate_race <- data_combined[!(is.na(mcnty) & is.na(cbsa) & is.na(cbsa_mcnty_code) & is.na(state)) & !is.na(year) & !is.na(sex) & !is.na(race) & !is.na(age_bin) & !is.na(prostate) & !is.na(edu_code) & !is.na(marital_ushd) & !is.na(weights)]
if (nrow(data_missing_prostate_race) + nrow(data_combined_prostate_race) != nrow(data_combined)) {
  warning(paste0("The total number of rows in the missingness and complete-case data sets for race/ethnicity (", nrow(data_missing_prostate_race) + nrow(data_combined_prostate_race), ") does not equal the number of rows in the original data set (", nrow(data_combined), "). Please check this." ))
}
data_missing_survive_prostate_race <- data_combined[(is.na(mcnty) & is.na(cbsa) & is.na(cbsa_mcnty_code) & is.na(state)) | is.na(year) | is.na(sex) | is.na(race) | is.na(age_bin) | is.na(survive_prostate) | is.na(edu_code) | is.na(marital_ushd) | is.na(weights)]
data_combined_survive_prostate_race <- data_combined[!(is.na(mcnty) & is.na(cbsa) & is.na(cbsa_mcnty_code) & is.na(state)) & !is.na(year) & !is.na(sex) & !is.na(race) & !is.na(age_bin) & !is.na(survive_prostate) & !is.na(edu_code) & !is.na(marital_ushd) & !is.na(weights)]
if (nrow(data_missing_survive_prostate_race) + nrow(data_combined_survive_prostate_race) != nrow(data_combined)) {
  warning(paste0("The total number of rows in the missingness and complete-case data sets for race/ethnicity (", nrow(data_missing_survive_prostate_race) + nrow(data_combined_survive_prostate_race), ") does not equal the number of rows in the original data set (", nrow(data_combined), "). Please check this." ))
}
data_missing_survive_breast_race <- data_combined[(is.na(mcnty) & is.na(cbsa) & is.na(cbsa_mcnty_code) & is.na(state)) | is.na(year) | is.na(sex) | is.na(race) | is.na(age_bin) | is.na(survive_breast) | is.na(edu_code) | is.na(marital_ushd) | is.na(weights)]
data_combined_survive_breast_race <- data_combined[!(is.na(mcnty) & is.na(cbsa) & is.na(cbsa_mcnty_code) & is.na(state)) & !is.na(year) & !is.na(sex) & !is.na(race) & !is.na(age_bin) & !is.na(survive_breast) & !is.na(edu_code) & !is.na(marital_ushd) & !is.na(weights)]
if (nrow(data_missing_survive_breast_race) + nrow(data_combined_survive_breast_race) != nrow(data_combined)) {
  warning(paste0("The total number of rows in the missingness and complete-case data sets for race/ethnicity (", nrow(data_missing_survive_breast_race) + nrow(data_combined_survive_breast_race), ") does not equal the number of rows in the original data set (", nrow(data_combined), "). Please check this." ))
}


######## 26. Collapse prostate cancer by county-year-age-sex and either race/ethnicity or education
data_combined_agg_prostate_race <- data_combined_prostate_race[, list(prostate_count = sum(prostate), prostate_weighted_count = weighted.mean(prostate, weights) * .N, weights = sum(weights), sample_size = .N), by = list(mcnty, cbsa_mcnty_code, state, year, sex, race, age_bin, edu_code, marital_ushd)]
stopifnot(nrow(data_combined_agg_prostate_race[is.na(prostate_count)]) == 0 & nrow(data_combined_agg_prostate_race[is.na(prostate_weighted_count)]) == 0)

data_combined_agg_survive_prostate_race <- data_combined_survive_prostate_race[, list(survive_prostate_count = sum(survive_prostate), survive_prostate_weighted_count = weighted.mean(survive_prostate, weights) * .N, weights = sum(weights), sample_size = .N), by = list(mcnty, cbsa_mcnty_code, state, year, sex, race, age_bin, edu_code, marital_ushd)]
stopifnot(nrow(data_combined_agg_survive_prostate_race[is.na(survive_prostate_count)]) == 0 & nrow(data_combined_agg_survive_prostate_race[is.na(survive_prostate_weighted_count)]) == 0)

data_combined_agg_survive_breast_race <- data_combined_survive_breast_race[, list(survive_breast_count = sum(survive_breast), survive_breast_weighted_count = weighted.mean(survive_breast, weights) * .N, weights = sum(weights), sample_size = .N), by = list(mcnty, cbsa_mcnty_code, state, year, sex, race, age_bin, edu_code, marital_ushd)]
stopifnot(nrow(data_combined_agg_survive_breast_race[is.na(survive_breast_count)]) == 0 & nrow(data_combined_agg_survive_breast_race[is.na(survive_breast_weighted_count)]) == 0)

#### Save collapsed data sets
saveRDS(data_missing_prostate_race, file = paste0(output_dir, "/brfss_agg_missing_data_prostate_race.rds"))
saveRDS(data_combined_agg_prostate_race, file = paste0(output_dir, "/brfss_agg_prostate_race.rds"))
saveRDS(data_missing_survive_prostate_race, file = paste0(output_dir, "/brfss_agg_missing_data_survive_prostate_race.rds"))
saveRDS(data_combined_agg_survive_prostate_race, file = paste0(output_dir, "/brfss_agg_survive_prostate_race.rds"))
saveRDS(data_missing_survive_breast_race, file = paste0(output_dir, "/brfss_agg_missing_data_survive_breast_race.rds"))
saveRDS(data_combined_agg_survive_breast_race, file = paste0(output_dir, "/brfss_agg_survive_breast_race.rds"))
rm(data_combined_prostate_race, data_missing_prostate_race, data_combined_agg_prostate_race, data_combined_survive_prostate_race, data_combined_survive_breast_race, data_missing_survive_prostate_race, data_combined_agg_survive_prostate_race, data_missing_survive_breast_race, data_combined_agg_survive_breast_race)
gc()


######## 27. Create a data set for analyzing diabetes
#### Drop observations without mcnty, year, sex, age, race and/or education; save these to separate files for follow-up
...
######## 28. Collapse diabetes by county-year-age-sex and either race/ethnicity or education
...
#### Load shapefiles
cbsa_pops <- readRDS(paste0("FILEPATH"))

year_versions <- unique(cbsa_pops[, c("year", "version", "is_metropolitan_division")])[order(year)]
shapes <- vector(mode = "list", length = nrow(year_versions))
shapes_fortified <- vector(mode = "list", length = nrow(year_versions))

for (i in 1:length(shapes)) {
  current <- year_versions[i]
  current_shape <- readRDS(paste0("FILEPATH"))
  shapes[[i]] <- spTransform(current_shape, "+init=epsg:4326") # Project shapefile
  shapes_fortified[[i]] <- fortify(shapes[[i]], region = "cbsa_mcnty_code") # Fortify SpatialPolygonsDataFrame to facilitate plotting
}
...
#### List outcomes
outcomes <- c("gen_health_45", "frequent_activity_limitations", "asthma", "copd", "arthritis", "depression", "tooth_loss", "asthma_now", "anxiety", "fallinj", "dementia", "prostate", "diabetes", "survive_prostate", "survive_breast")
...
for (outcome in outcomes) {
  ...
  current <- direct_estimates[marital == 9 & edu == 9 & age == 98 & level == "natl" & sex != 3]
  print(ggplot(data = current) + theme_bw() +
          ...
          labs(x = "Year", y = "Prevalence", title=paste0("Prevalence of ", outcome, " (Direct Estimates from BRFSS), National"), subtitle = paste0("Ages 20+.\nAll education and marital status categories.")) +
  ...
  current <- direct_estimates[marital == 9 & edu == 9 & age == 98 & level == "natl" & year != -1 & sex == 3]
  print(ggplot(data = current) + theme_bw() +
          geom_line(data = current, aes(x = year, y = get(outcome), color = factor(race), linetype = factor(sex))) +
          geom_point(data = current, aes(x = year, y = get(outcome), color = factor(race), size = sample_size), alpha = 0.25) +
          labs(x = "Year", y = "Prevalence", title=paste0("Prevalence of ", outcome, " (Direct Estimates from BRFSS), National"), subtitle = paste0("Ages 20+.\nAll education and marital status categories.")) +
  ...
  current <- direct_estimates[marital == 9 & edu == 9 & !(age %in% c(98)) & level == "natl" & year == max(direct_estimates$year) & sex != 3]
  print(ggplot(data = current) + theme_bw() +
          ...
          labs(x = "Age", y = "Prevalence", title=paste0("Prevalence of ", outcome, " (Direct Estimates from BRFSS), National"), subtitle = paste0(max(direct_estimates$year), "\nAll education and marital status categories.")) +
          theme(plot.title = element_text(hjust = 0.5, face = "bold"), plot.subtitle = element_text(hjust = 0.5)) +
          scale_color_discrete(name = "Race", labels = race_labels) + scale_linetype_discrete(name = "Sex", labels = sex_labels) + scale_size_continuous(name = "Sample Size"))
  ...
  limits <- quantile(direct_estimates[marital == 9 & edu == 9 & age %in% 98 & sex != 3 & race == 1 & level == "cbsa_mcnty", get(outcome)], probs = c(0, 1), na.rm = TRUE)

  for (i in 1:nrow(year_versions)) {
    current <- direct_estimates[marital == 9 & edu == 9 & age %in% 98 & level == "cbsa_mcnty" & year == year_versions[i, year]]
    if (nrow(current) == 0) {
      next
    }

    current_map <- shapes_fortified[[i]]
    ...
    print(ggplot() + theme_map() + geom_map(data = current[sex != 3 & race == 1], aes(map_id = area, fill = get(outcome)), map = current_map) +
            ...
            labs(title=paste0("Prevalence of ", outcome, " (Direct Estimates from BRFSS), CBSA-Mcnty-Level"), subtitle = paste0(year_versions[i, year], ", CBSA version ", year_versions[i, version], ", is_metropolitan_division = ", year_versions[i, is_metropolitan_division], ".\nAll ages (represented in data set for this indicator), races, education and marital status categories."), fill = "Prevalence") +
            scale_fill_gradientn(colours = rev(brewer.pal(5, "Spectral")), n.breaks = 5, limits = limits))
  }

  limits <- quantile(direct_estimates[marital == 9 & edu == 9 & age %in% 98 & year != -1 & sex == 3 & race != 1 & level == "cbsa_mcnty", get(outcome)], probs = c(0, 1), na.rm = TRUE)
  for (i in 1:nrow(year_versions)) {
    current <- direct_estimates[marital == 9 & edu == 9 & age %in% 98 & level == "cbsa_mcnty" & year == year_versions[i, year]]
    if (nrow(current) == 0) {
      next
    ...
    print(ggplot() + theme_map() + geom_map(data = current[sex == 3], aes(map_id = area, fill = get(outcome)), map = current_map) +
            ...
            expand_limits(x = state_shape$long, y = state_shape$lat) +
            theme(plot.title = element_text(hjust = 0.5, face = "bold"), plot.subtitle = element_text(hjust = 0.5)) + theme(legend.position = "right") + coord_map(projection = "albers", lat0 = 29.5, lat1 = 45.5) +
            labs(title=paste0("Prevalence of ", outcome, " (Direct Estimates from BRFSS), CBSA-Mcnty-Level"), subtitle = paste0(year_versions[i, year], ", CBSA version ", year_versions[i, version], ", is_metropolitan_division = ", year_versions[i, is_metropolitan_division], ".\nAll ages (represented in data set for this indicator), sexes, and education and marital status categories."), fill = "Prevalence") +
            scale_fill_gradientn(colours = rev(brewer.pal(5, "Spectral")), n.breaks = 5, limits = limits))
  }

  #### Plot state-level maps of prevalence by year
  limits <- quantile(direct_estimates[marital == 9 & edu == 9 & age %in% 98 & sex != 3 & race == 1 & level == "state", get(outcome)], probs = c(0, 1), na.rm = TRUE)
  for (y in years) {
    current <- direct_estimates[marital == 9 & edu == 9 & age %in% 98 & race == 1 & level == "state" & year == y]
    if (nrow(current) == 0) {
      next
    }
    ...
    print(ggplot() + theme_map() + geom_map(data = current[sex != 3 & race == 1], aes(map_id = area, fill = get(outcome)), map = states) +
            ...
            expand_limits(x = state_shape$long, y = state_shape$lat) +
            theme(plot.title = element_text(hjust = 0.5, face = "bold"), plot.subtitle = element_text(hjust = 0.5)) + theme(legend.position = "right") + coord_map(projection = "albers", lat0 = 29.5, lat1 = 45.5) +
            labs(title=paste0("Prevalence of ", outcome, " (Direct Estimates from BRFSS), State-Level, ", y), subtitle = paste0("All ages (represented in data set for this indicator), races, education and marital status categories."), fill = "Prevalence") +
            scale_fill_gradientn(colours = rev(brewer.pal(5, "Spectral")), n.breaks = 5, limits = limits))
  }
  dev.off()
}
