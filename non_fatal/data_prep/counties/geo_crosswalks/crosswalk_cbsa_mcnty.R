...
#### Database upload arguments
# get the best PS frame
ps_frame_meta <- get_post_stratification_frame(post_stratification_type_id = 1, # no phone
                                               get_best = TRUE)
post_stratification_frame_id = ps_frame_meta$post_stratification_frame_id
# get the best survey extraction version

survey_extraction_meta <- get_survey_extraction(input_type = "brfss", get_best = T)

# load NIDS for the Census delineation files, which are used to define CBSA-mcnty mapping
nids <- as.list(fread("FILEPATH")$NID)

#### Read in data
locs <- fread("FILEPATH")
locs[, cnty := sprintf("%05d", cnty)]

pops_fips_filepath <-"FILEPATH"
pops_fips <- readRDS(pops_fips_filepath)

cbsa_mcnty <- readRDS("FILEPATH")
ps_path <- ps_frame_meta$post_stratification_path
ps_path_under20 <- "FILEPATH"
stopifnot(dirname(ps_path) == dirname(ps_path_under20))

####  Load and process microdata
## Set paths
data_version <- basename(unique(survey_extraction_meta$output_file_path))
input_dir <- unique(survey_extraction_meta$output_file_path)
...
table(partials[, list(year, mcnty)])
...
uniqueN(coverage[present_in_data != N & present_in_data > 0, mcnty])
...
######## 8. Upload output metadata
mcnty_mapping_crosswalk_id <- save_mcnty_mapping_crosswalk(
  mcnty_mapping_type_id = 1,
  covariate_dataset_id = 167,
  mcnty_mapping_filepath = paste0(output_dir, "cbsa_mcnty_crosswalk.rds"),
  post_stratification_frame_id = post_stratification_frame_id,
  survey_extraction_version_id = unique(survey_extraction_meta$survey_extraction_version_id), 
  nids = nids,
  is_best = FALSE
)
