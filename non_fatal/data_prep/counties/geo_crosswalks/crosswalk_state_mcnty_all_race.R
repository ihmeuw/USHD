###########################################################################################################################################
## Description: Determine age-sex-year-state-specific population proportions represented by each merged county to be used for all-race
##              state-level models to predict all-cause ylds
##
...
#### Derive state-level populations by year-sex-age-edu-marital
state_pops <- ps_frame[, list(state_pop = sum(value), state_pop_age_pooled = sum(value_age_pooled)), by = list(state, year, sex, age)]

#### Merge state pops onto mcnty pops
pops <- merge(ps_frame[, c("age", "sex", "year", "mcnty", "value", "value_age_pooled", "state")], state_pops, by = c("state", "year", "sex", "age"), all = TRUE)
...
pops[, pop2 := sum(pop), by = c("mcnty", "state", "age", "sex")]
pops[, state_pop2 := sum(pop), by = c("state", "age", "sex")]
pops[, wt2 := pop2 / state_pop2]

# Third level of the cascade (drop year, and use pooled age groups)
pops[, pop3 := sum(pop_age_pooled), by = c("mcnty", "state", "sex", "age")]
pops[, state_pop3 := sum(pop_age_pooled), by = c("state", "sex", "age")]
pops[, wt3 := pop3 / state_pop3]

# Fourth level of the cascade (drop year, race, and sex)
pops[, pop4 := sum(pop_age_pooled), by = c("mcnty", "state", "age")]
pops[, state_pop4 := sum(pop_age_pooled), by = c("state", "age")]
pops[, wt4 := pop4 / state_pop4]

# Fifth level of the cascade (drop year, race, sex, and age)
pops[, pop5 := sum(pop_age_pooled), by = c("mcnty", "state")]
pops[, state_pop5 := sum(pop_age_pooled), by = c("state")]
...
stopifnot(nrow(pops[state_pop5 < 20]) == 0)
stopifnot(pops[, sum(agg_wt), by = c("state", "year", "sex", "age")][, max(abs(V1 - 1))] < 1e-10)
table(pops$use_version)

#### Cleanup
pops[, c("use_version", "wt", "wt0", "pop_age_pooled", "state_pop_age_pooled", "pop2", "state_pop2", "wt2", "pop3", "state_pop3", "wt3", "pop4", "state_pop4", "wt4", "pop5", "state_pop5", "wt5") := NULL]
