...
if (interactive()) {
  ...
  dir <- "FILEPATH"
  edu_race_comparison_dir <- "FILEPATH"
  plot_level <- "state"
  initial_level <- 0
  terminal_level <- 0
...
valid_measures <- c("mx", "yll", "yld", "pred")
...
if (any(measures %in% c("mx", "yll"))) {
  ...
  lu_modeldir <- paste0("FILEPATH", LU_folder, "/USHD_INTERMEDIATE/MODELS/", data_folder)
} else if(identical(measures, "pred")){
  lu_modeldir <- gsub(pattern = "FILEPATH",
...
geo_diff_dir <- paste0(dir, "/check_raked_results/geo_diff")
lsae.utils::make_group_writable_dir(geo_diff_dir)

cause_diff_dir <- paste0(dir, "/check_raked_results/cause_diff")
lsae.utils::make_group_writable_dir(cause_diff_dir)

race_diff_dir <- paste0(dir, "/check_raked_results/race_diff")
lsae.utils::make_group_writable_dir(race_diff_dir)

edu_diff_dir <- paste0(dir, "/check_raked_results/edu_diff")
...
print_output_from_foreach <- function(check_status) {
  # foreach returns error objects when something breaks, which splits apart when unlist is called into
  # a named list with '.message' and '.call' components, corresponding with the error message and the lowest
  # level traceback call (which is not particularly useful for debugging)
...
for (i in initial_level:terminal_level) {
  ...
  # foreach settings
  capture.output(cl <- makeCluster(threads, type = "FORK", outfile = ""), file = "/dev/null")
  ...
  cause_status <- foreach(this_parent = parent_causes_to_check_cause, .packages = c("data.table"), .errorhandling = "pass") %dopar% {
    cause_status_sub <- foreach(measure = measures, .errorhandling = "pass") %do% {
      ...
      child_est <- rbindlist(lapply(children[, acause], function(this_child) {
        ...
        if (!causes[acause == this_child, compile_model]) {
          ...
          return()
        } else if (!file.exists(child_path)) {
      ...
      child_est <- child_est[, list(child_value = sum(value_mean)), keyby = "level,area,year,sex,race,age"]
      ...
      parent_est <- readRDS(parent_path)
      setnames(parent_est, paste0(measure, "_mean"), "value_mean")
      parent_est <- parent_est[, list(level, area, year, sex, race, age, parent_value = value_mean)]

      # merge and compare
      message(measure, " ", this_parent, " - Merging and comparing")
      est <- merge(child_est, parent_est, by = c("level", "area", "year", "sex", "race", "age"))
    ...
    saveRDS(cause_status_sub, file = paste0(results_dir, "/cause_diff/", measure, "_cause_status_", this_parent, "_lvl_", i, ".rds"))
  ...
  geo_status <- foreach(this_cause = parent_causes_to_check, .packages = c("data.table"), .errorhandling = "pass") %dopar% {
    geo_status_sub <- foreach(measure = measures, .errorhandling = "pass") %do% {
      ...
      } else if (!file.exists(lower_path)) {
      ...
      lower <- readRDS(lower_path)
      # generalize column names to work for both yll and mx
      setnames(lower, c(paste0(measure, c("_mean", "_lb", "_ub", "_se"))), c("value_mean", "value_lb", "value_ub", "value_se"))

      lower <- lower[J(raking_area_var), list(area, year, sex, race, edu, age, value_mean, value_lb, value_ub)]
    ...
    saveRDS(geo_status_sub, file = paste0(results_dir, "/geo_diff/", measure, "_geo_status_", this_cause, "_lvl_", i, ".rds"))
  ...
  if (!identical("yld", measures)) {
    ...
    plot_status <- foreach(this_cause = parent_causes_to_check, .packages = c("data.table", "ggplot2", "RColorBrewer", "gridExtra"), .inorder = T, .errorhandling = "pass", .final = function(x) setNames(x, parent_causes_to_check)) %dopar% {
      ...
      foreach(measure = measures, .errorhandling = "pass") %do% {
        ...
        if(measure == "pred") keep_levels <- unique(c(keep_levels, "natl"))
        pre <- pre[
          J(keep_levels),
        ...
        if (!identical(measure, "pred") && dir.exists(edu_race_comparison_dir)) {
        ...
        if(identical(measure, "pred")) {
          ...
          for(aa in c(98, 99)){
            print(ggplot(fdata[age == aa], aes(x = year, y = value)) +
                    ...
                    facet_wrap(~race) +
                    labs(
                      title = paste0(ifelse(aa == 98, "Crude ", "Age-standardized "), measure_name, " rate by ", plot_level, ", ", this_cause, " (National)"),
                      x = "Year", y = measure
                    ) +
        ...
        message(glue::glue("Plot type 1: crude/age-standardized {measure_name} rate by {plot_level}, and race/edu. One at the county level and one at the county-race/edu level."))
          ...
          for (race_sub in race_iterations) {
            ...
            if (!skip_all_race && (race_sub == race_default & edu_sub == edu_default)) { # selecting county level data
              ...
              for (aa in c(98, 99)) {
                print(ggplot(fdata_county[age == aa, ], aes(x = year, y = value)) +
                        ...
                        labs(
                          title = paste0(ifelse(aa == 98, "Crude ", "Age-standardized "), measure_name, " rate by ", plot_level, ", ", this_cause, ", race ", race_sub, ", edu", edu_sub, " (county model)"),
                          x = "Year", y = measure
                        ) +
            ...
            for(aa in c(98, 99)){
              ...
              print(ggplot(fdata[age == aa], aes(x = year, y = value)) +
                      ...
                      facet_wrap(~level_area, scales = "free_y") +
                      labs(
                        title = paste0(ifelse(aa == 98, "Crude ", "Age-standardized "), measure_name, " rate by ", plot_level, ", ", this_cause, ", race ", race_sub, ", edu", edu_sub),
                        x = "Year", y = measure
                      ) +
                      theme_bw(base_size = 12) +
                      theme(axis.text.x = element_text(angle = 45, hjust = 1)))
        ...
        message(glue::glue("Plot type 2: both-sex, state-level, age-specific {measure_name} rate for each state, and race/edu, over time."))
        for (edu_sub in edu_iterations) {
          for (race_sub in race_iterations) {
            ...
            for (this_area in unique(fdata[, area])) {
              print(
                ggplot(fdata[area == this_area,], aes(
                  ...
                  labs(
                    title = paste0(
                      measure_name, " rate by age, ", this_cause, ", ", raking_area_var, " ", this_area, ", race ", race_sub, ", edu ", edu_sub),
                    x = "Year",
                    y = measure
                  ) +
                  theme_bw(base_size = 12) +
        ...
        # make maps
        if (!skip_all_race) {
          message(glue::glue("Plot type 3: Map of raked county-level model age-standardized {measure_name} rate, both-sex, {area_var}-level, for latest year."))
          ...
          print(ggplot(fdata_mcnty) + facet_grid(. ~ type) +
                  ...
                  theme_bw(base_size = 12) +
                  labs(title = paste0(
                    "county-level model, age-standardized ", measure_name," rates, ", max(years),
                    ", both sexes"
                  ), caption = "All-race and All-edu"))
        } # end skip_all_race if statement

        message(glue::glue("Plot type 4: Map of raked and unraked model age-standardized {measure_name} rate, both-sex, {area_var}-level, for each year and race/edu. Also, map of differences (abs and rel) between raked and unraked."))

        for(yr in range(years)){
          for (edu_sub in c(edu_iterations, NA)) {
            if(identical(measure, "pred") & is.na(edu_iterations)) next # skip the edu loops for RF if the files are "all edu"
            for (race_sub in c(race_iterations, NA)) {
              ...
              print(ggplot(fdata) +
                      ...
                      theme_bw(base_size = 12) +
                      labs(
                        title = paste0("age-standardized ", measure_name, " rates, ", yr, ", both sexes"),
                        subtitle = paste0("Race ", race_sub, ", Education, ", edu_sub)
                      ))

              # map difference b/w raked and unraked
              fdata <- est[J(level = area_var, year = yr, sex =3, age = 99, race = r_tmp, edu = e_tmp),
                           on = c("level", "year", "sex", "age", "race", "edu")]

              fdata[, type := factor(type, levels = c("Unraked", "Raked"))]
              fdata <- dcast(fdata, ...~type, value.var = c("value", "ci")) # what is ...~?
              ...
              print(ggplot(fdata) +
                      ...
                      facet_wrap(edu ~ race) +
                      labs(
                        title = paste0("Raked - Unraked: age-standardized ", measure_name, " rates, ", yr, ", both sexes"),
                        subtitle = paste0("Race ", race_sub, ", Education", edu_sub)
                      ))
        ...
        if(!skip_all_race){
          message(glue::glue("Plot type 5: Comparison scatters of raked vs unraked age-standardized {measure_name} rate, both-sex, {area_var}-level, for each year, all-race/all-edu."))
          ...
          print(ggplot(fdata_mcnty, aes(x = Unraked, y = Raked)) + facet_wrap(~year, nrow = 4) +
                  ...
                  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
                  labs(title = paste0(
                    "County-level model, age-standardized ", measure_name, " rates by\n",
                    area_var, ", unraked and raked, ", this_cause
                  )) +
                  theme_bw(base_size = 12) + theme(axis.text.x = element_text(angle = 45, hjust = 1)))
        }

        message(glue::glue("Plot type 6: Comparison scatters of raked vs unraked age-standardized {measure_name} rate, both-sex, {area_var}-level, for each year, and race/edu."))
        ...
        for (edu_sub in edu_iterations) {
          for (race_sub in race_iterations) {
            ...
            print(ggplot(fdata, aes(x = Unraked, y = Raked)) +
                    ...
                    theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
                    labs(
                      title = paste0("Age-standardized ", measure_name, "\nrates by ", area_var, ", unraked and raked, ", this_cause),
        ...
        for (edu_sub in edu_iterations) {
          for (race_sub in race_iterations) {
            ...
            print(grid.arrange(p1, p2, ncol = 2, top = paste0(
              "CI width for age-standardized ", measure, " rates by ", area_var,
...
lsae.utils::send_email("Validation step complete", "PROJECT", "...")
