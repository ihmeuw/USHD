####################################################################################################
##
## Description: Rake mortality or Years Lived with Disability (yld) to sum up properly to parent 
##                cause/race/ethnicity       
##
...
if (to_dir == "gbd") {
  ...
  to_settings <- copy(from_settings)
...
if(not_all_child_causes_modeled) {
  ...
  # So, to do this we will merge on the missing causes
