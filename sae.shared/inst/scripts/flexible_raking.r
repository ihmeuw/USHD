...
library(lbd.loader, lib.loc = sprintf("FILEPATH/lbd.loader-%s.%s", R.version$major, strsplit(R.version$minor, '.', fixed = TRUE)[[1]][[1]]))
...
if (interactive()) {
  ...
  from_geo <- "mcnty"
  to_geo <- "state"
  common_raking_vars <- c("year", "sex", "age", "sim") # equal to the :shared_demographics" in the old scripts
  lower_raking_vars <- c("year", "sex", "age", "sim", "mcnty", "race") # equal to "lower_demographics" in the old scripts
  upper_raking_vars <- c("year", "sex", "age", "sim", "state") # equal to "geography_demographics" in the old scripts
...
if(!is.null(from_settings$settings$by_source) && from_settings$settings$by_source == TRUE){
  lower_draws <- lower_draws[source_v2 == from_settings$settings$gold_standard]
  lower_draws[, source_v2 := NULL]
} else if(any(grepl("source", names(lower_draws)))){

  stop("There is a source column in the draws, but the settings do not specify by_source = TRUE")
}
...
# HACK - we need 'race' in the population data frame here.
...
# Pass REI so that calc_all_ages (called within save_draws_and_estimates) can 
# subset to 20+ for age-standardization, when appropriate for the risk factor.
from_settings$save_draws_and_estimates(
  draws = draws,
  acause = cause_name,
  measure = measure,
  rei_id = from_settings$settings$rei_id,
  year = year,
  sex = sex,
  type = 'raking'
)
