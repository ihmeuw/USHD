...
if (interactive()) {
  ...
  target_dims <- c("race","cause")
  parent_acause <- "mental_drug" #' _subs'
  
  to_geo <- "mcnty"
  draws <- 1000
  age <- 1
  common_raking_vars <- c("year","age","sex","sim","mcnty")
...
if (draws != 1000) {
  warning(sprintf("draws is %i. Only draws 0-%i will be used. Raked results will not match GBD properly", draws, draws - 1))
}
...
# These will always be USHD draws.
...
# These draws can either be USHD or GBD draws, hence the difference in the logic for the 'raked' argument.
...
if (nrow(upper_parent_cause_draws[value != 0]) == 0) {
  ...
  # There are two cases where this might not happen: 
  # 1. When dealing with unmodeled age/sex combinations for an available cause, like mater_neonat, or 
  # 2. When GBD does not have a cause that USHD models. 
  # In the first case, this is acceptable - work around the available draws. 
  # In the second case, throw an error. 
  ...
  # run the raking function iteratively until convergence
  # Constant columns are the demographics shared across lower and upper draws
  stop.if.not.expected.cols(raking_val,
                            ...
                            extra.ok = TRUE
  ) # These columns are required for raking to work.
