...
##
## Description: Calculate mx, yll, yld, pred (bmi) and (optionally) life table draws for both sexes combined by
...
##              mx, yll, yld, pred, and lt draws [if lt == T] by sex:
...
##
## Outputs:     mx, yll, yld, pred, and lt [if lt == T] draws and estimates for both sexes combined:
##                "[dir]/mx_draws_[level]_[year]_3_[race].rds"
##                "[dir]/mx_est_[level]_[year]_3_[race].rds"
##                "[dir]/yll_draws_[level]_[year]_3_[race].rds"
##                "[dir]/yll_est_[level]_[year]_3_[race].rds"
##                "[dir]/yld_draws_[level]_[year]_3_[race].rds"
##                "[dir]/yld_est_[level]_[year]_3_[race].rds"
##                "[dir]/draws_[level]_[year]_3_[race]_[edu].rds" (for pred)
##                "[dir]/est_[level]_[year]_3_[race]_[edu].rds" (for pred)
##                "[dir]/lt_draws_[level]_[year]_3_[race].rds"
##                "[dir]/lt_est_[level]_[year]_3_[race].rds"
##                (or the corresponding raked files if raked == T)
##
####################################################################################################
...
if (any(is.na(wts))) {
  stop("There are NA weights in wts.")
}
...
agg_sex <- function(acause, year, race, edu, wts, std_wt, var, raked) {
  ...
  # Use the saver object to load draws with level (mcnty, state, or natl) in the file name
...
if (run_lt) {
  ...
  # Use the saver object to load lt draws with level (mcnty, state, or natl) in the file name
  draws <- saver$load_draws(
    acause = acause,
    measure = 'lt',
    year = year,
  ...
  draws[, acause := "_all"] # do this again to get the correct order
  saver$io_engine$save_draws(
    draws = draws,
    acause = "_all",
    save_dir = saver$dir,
