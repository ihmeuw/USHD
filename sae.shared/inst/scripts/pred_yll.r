...
##              lt draws ("[dir]/lt_draws_[area_var]_[year]_[sex]_[race]_[edu].rds", or the
##                corresponding raked version).
...
##                "[dir]/yll_est_[area_var]_[year]_[sex]_[race]_[edu].rds"
...
if (!file.exists(lt_file)) stop_or_quit(sprintf("life table file %s was not found!", lt_file), status = 9)
...
if (any(is.na(draws$death_age))) stop_or_quit("NAs found in death_age - will create issues producing life tables!", status = 20)
...
# population
pop <- load_population(settings$settings$pop_file)
...
draws[, yll := mx * ex]
if (any(is.na(draws$yll))) stop_or_quit("NAs produced in YLL calculation", status = 20)
...
draws <- calc_all_ages(draws, std_wt, "yll", merge_vars)
