...
rake <- function(data, agg_var, constant_vars, replace_value = F, weight_pops = T) {
  ...
  # Raise an error if agg_var is not state -- may not work for other types of raking
    ...
    # if replace_value = F & any raking_weight is inf, give an error. The way this is currently set up, we
    # overwrite the original value with the raked value when raking_weight is inf.
    # This can be improved if needed.
    if (!replace_value & any(is.infinite(data$raking_weight))) {
      stop("Infinite raking weights detected. This is likely due to all county counts
            being zero, but the state counts being non-zero. 
            To avoid convergence issues here, we replace original value with a raked 
            val consistent with GBD. replace_value = FALSE was passed, but this is 
