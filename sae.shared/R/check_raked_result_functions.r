...
check_race_edu_diff <- function(race_edu_diff, results_dir) {
  ...
  if (nrow(race_edu_diff) > 0) {
    ...
    msg_list <- append(msg_list, "Race/edu-checks not done. Caution, diff-files missing. Check /check_raked_result/edu_status.rds or /check_raked_result/race_status.rds")
  }
  result <- list(
    messages = msg_list,
    failed = failed_causes,
