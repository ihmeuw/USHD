#' @title submit_single_bmi_aggregation
#' @description A function version of the _submit_single_cause.r script used for submitting jobs to 
#'              aggregate based on the type passed in.
#'
...
#'
#' @export
submit_single_pred_aggregation <- function(jobmon_workflow,
                                          jobmon_tool,
                                          dir,
                                          testing,
                                          model_settings = NULL,
  ...
  settings_env <- model_settings$settings
  ...
  cause <- basename(dirname(dirname(dir)))

  # skip mx, yll and yld aggregation
  agg_skip_flags <- c(CLI.Flags$skip_yll_aggregation, CLI.Flags$skip_mx_aggregation, CLI.Flags$skip_yld_aggregation)
  ...
  if (settings_env$by_race) {
    ...
    agg_races_tasks[sex != 3,
      `:=`(agg_races = list(jobmon_add_task(
        ...
        tool = jobmon_tool,
        name = unique(paste("agg_races", year, sex, raked, cause, sep = "_")),
  ...
  compile_tasks[, `:=`(compile = list(
    jobmon_add_task(
      ...
      tool = jobmon_tool,
      name = unique(paste("agg_compile", cause, sep = "_")),
