...
#' @export
collapse_draws_lt <- function(draws, dir, id_vars = c("level", "area", "year", "sex", "race", "edu", "age")) {
  ...
  est[, c("qx_mean", "ex_mean", "ax_mean") := lifetable(mx = mx_mean, ax = ax_mean, graduation = F, 
                                                        extrap = F, lt_hc = settings$lt_hc, gr = mean_gr)[, list(qx, ex, ax)], 
      by = id_vars2]
  
  est[,mean_gr := NULL]


  # if mx is so high that mx -> qx leads to qx > 1, ax is adjusted downward. Often
  ...
  # upper bounds
  est[abs(ax_ub - ax_lb) < 1e-10 & !between(ax_mean, ax_lb, ax_ub), ax_mean := (ax_lb + ax_ub)/2]
  
  # ex_mean can sometimes be outside the UI bounds when it is recalculated
