...
#' 3. If 'area/level' are specified as the geographic variable, rename to 'FILEPATH' etc.
...
Schema <- R6::R6Class(
  ...
  public = list(
    ...
    drop_additional_columns = function(results) {
      ...
      # HACK - if 'adjusted' in data and 'ushd.use_adjusted_data' is NA, 
      # keep adjusted in data. We need to formalize this with the idea of 'optional columns' soon. 
    ...
    validate = function(data, measure, value_col, drop_additional_columns = TRUE) {
      ...
      for (col in names(self$required_cols)) {
        # lapply(draws, class)$age is faster than class(draws[,age])
