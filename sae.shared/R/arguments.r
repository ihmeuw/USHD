...
add_aggregation_skip_flags <- function(parser) {
  ...
  parser$add_argument(CLI.Flags$skip_pred_aggregation, action = "store_true", default = FALSE, help = "Add this flag to skip pred (BMI-Risk) aggregation")
}
...
#' Add measure_id argument
#' @export
add_measure_id_argument <- function(parser) {
  parser$add_argument(
    "--measure_id", 
    type = "integer", 
    choices = 1:4, 
    help = "1 for death, 2 for daly, 3 for yld, 4 for yll"
  )
}

#' Add location_id arg
#' @export
add_location_id_argument <- function(parser) {
  parser$add_argument("--location_id", type = "character", help = "Location id of to_geo (state) geography level")
}
...
validate_cause_subset_args <- function(children_of, initial_level, terminal_level) {
  if (!is.null(children_of) & (!is.null(initial_level) | !is.null(terminal_level))) {
    lsae.utils::stop_or_quit("Parent cause subset and initial level/terminal level cannot be specified at the same time.", status = 2)
  }
}
...
#' @export
get_args <- function() {
  # by default argparse uses commandArgs(trailingOnly = TRUE)
  args <- commandArgs(TRUE)
  # however this fails if '--args' is not in the CLI call
  ...
  if (is.null(args) || length(args) == 0) {
    ...
    if (is.na(idx)) {
      ...
      # skip the immediate next arg (the script - _submit_single_cause.r) and give only the arguments
