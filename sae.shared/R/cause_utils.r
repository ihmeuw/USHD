...
add_excluded_cause_draws <- function(data, removed_draws, causes_df, lower_demographics, geography_demographics, geoagg_file) {
  ...
  if(diff_cols == "race") {
    ...
    removed_draws[,(diff_cols) := NULL]
  } else if (!('state' %in% geography_demographics)) {
    stop("There is a value in lower_demographics that is not in geography_demographics, but it is not race or edu. Also, 'state' is not in geography_demographics, so we don't expect this difference to be due to geography. Not sure how to handle this")
  }
  
  
  # We are having trouble here where the schema between 'square' and 'removed_draws' 
  # does not line up, and explodes. One is at the county-level, and one is at the state-level.
  # The solution is: 
