...
#' results are not guarenteed to be stable (or may be incorrect??), so an error is thrown.
#'
...
add_excluded_cause_draws <- function(data, removed_draws, causes_df, lower_demographics, geography_demographics, geoagg_file) {
  ...
  # We are having bugs here where the schema between 'square' and 'removed_draws' 
