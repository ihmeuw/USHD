...
get_consolidated_settings <- function(dir) {
  main_settings <- settings_from_get_settings(dir)
  ...
  for (cause in submitted_causes) {
    ...
    # include in submitted_causes if specific cause_setting has different setting than main_settings or are not
...
#'
#' @param dir [character] model directory
#' @param copy_J [logical] If TRUE, copies FILEPATH files used in the settings to FILEPATH and also updates
...
write_consolidated_settings <- function(dir, copy_J = FALSE) {
  ...
  if(copy_J) {
    ...
    message("FILEPATH paths in 'settings.yaml' are updated!")
  }
}
...
#'
#' @description Uses get_updated_settings_on_copy_J_dependencies method to copy FILEPATH files to FILEPATH, and also updates
...
update_settings_with_J_independency <- function(dir) {
  ...
  if (!file.exists(file.path(dir, "settings.yaml"))) {
    stop_or_quit(paste0("Copying files from FILEPATH requires yaml settings at: ", file.path(dir, "settings.yaml")),
                 status = 1
    )
  }
