...
#'
#' @param dir [character] model directory
#' @param copy_J [logical] If TRUE, copies files
...
#'
#' @description Uses get_updated_settings_on_copy_J_dependencies method to copy files, and also updates
...
update_settings_with_J_independency <- function(dir) {
  # copying files requires that you have settings.yaml file because that is where updated paths are written to.
  if (!file.exists(file.path(dir, "settings.yaml"))) {
    lsae.utils::stop_or_quit(paste0("Copying files requires yaml settings at: ", file.path(dir, "settings.yaml")),
      status = 1
    )
  }
