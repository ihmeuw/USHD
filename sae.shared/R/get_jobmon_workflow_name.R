...
get_jobmon_csv_filepath <- function(measure,
  ...
  if (!is.null(children_of)) {
    path <- paste0(from_dir, "/jobmon_ids/", type, "_", measure, "_", children_of, ".csv")
  } else {
    path <- paste0(from_dir, "/jobmon_ids/", type, "_", measure, "_", initial_level, "_", terminal_level, ".csv")
  }
  return(path)
}
...
#' looks for the relevant csv file (e.g. jobmon_ids/raking_mx_0_0.csv) saved out by a previous run to get
