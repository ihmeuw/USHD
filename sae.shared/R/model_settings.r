...
ModelSettings <- R6::R6Class(
  ...
  public = list(
    ...
    #' @field deaths_refresh_name [string] version name for deaths. cannot be specified at the same time as deaths_file
    deaths_refresh_name = NULL,
    ...
    #' @field year_knots_spec [integer] Specified year knots
    year_knots_spec = NULL,

    #' @field detach_2019_2020 [boolean] If you want to adjacency matrices for years to specify 2019 and 2020 are neighbors
    detach_2019_2020 = FALSE,
    ...
    #' @description Copies FILEPATH drive files listed on settings to dir/from_FILEPATH, updates the relative path in settings and returns it.
    #' This method also writes metadata of this task to dir/from_FILEPATH
    get_updated_settings_on_copy_J_dependencies = function() {
      make_output_dir(paste0(self$dir, "/from_FILEPATH"))
      copy_to_dir <- paste0(self$dir, "/from_FILEPATH")
      named_files <- c(
        "adjmat_file", "deaths_file", "geoagg_files", "shape_file", "age_std_file", "ref_lt_file"
      )

      message("Copying FILEPATH drive files.")
      metadata <- list()
      metadata <- append(metadata, paste0("Copying FILEPATH Drive files started at: ", Sys.time()))

      # copies file from FILEPATH drive to local drive and returns new path and metadata
          ...
          if (identical(named_file, "geoagg_files")) {
            ...
            for (named_sub_file in named_sub_files) {
              ...
              if (!is.null(new_path)) {
                ...
                # save original FILEPATH path to 'unused_j_paths' list object
      ...
      msg <- "Original FILEPATH drive paths are listed under 'unused_j_paths' !"
      message(msg)
      metadata <- append(metadata, msg)
      metadata <- append(metadata, paste0("Copying FILEPATH drive files done at: ", Sys.time()))
      ...
      message("Copying FILEPATH drive files done!")
    ...
    load_population = function(year, raking_area_var) {
      ...
      # Two scenarios where population should be filtered to certain ages:
      # 1. subset by age because bmi (pred) and has age groups 20:85 only
      # 2. subset by age because mortality by edu has age groups 25:85  only
      if ((!is.null(self$measure) && self$measure %in% c("pred", "ab")) | (self$by_edu)) {
        message(sprintf("Filtering population to ages %s", paste(self$ages, collapse = ', ')))
        all_age_pop <- all_age_pop[age %in% self$ages]
      }
    ...
    load_draws = function(acause, measure, year, sex, raked = "raked",
      ...
      if (identical(measure, "ab")) {
        raw.lower.draws <- self$process_lower_draws(raw.lower.draws, population, measure)
      }

      # if there are more draws in the lower draws than in self$n.sims, subset
      # to the first self$n.sims draws and issue a warning
      if (raw.lower.draws[, max(sim)] > self$n.sims) {
        warning("More draws in lower draws than expected. Subsetting to first ", self$n.sims, " draws.")
        raw.lower.draws <- raw.lower.draws[sim <= self$n.sims]
      }
    ...
    #'
    #' @description Converts lower draws from wide to long format. Converts cause_id
    #' to acause and age_group_ids to ages.  Only used for attributable_burden (see
    #' reference 10 lines above)
    #'
    ...
    #' @return data.table
    #' @export
    process_lower_draws = function(raw.lower.draws, population, measure) {
      ...
      ages <- self$ages
      age_group_ids <- translate_ages(ages, "age_group_id", measure)
      ...
      # No easy way to translate age_group_ids from lower_draws to ages.
      ...
      # If sim is a factor, subset to the draw number contained in the label string
      # before converting to integer (otherwise, factor ordered alphabetically,
      # which will make draw_10 come before draw_2, etc.)
      if(draws[, is.factor(sim)]){
        # extract the number from sim
        draws[, sim := as.integer(gsub("draw_", "", sim))]
        # if the min sim is 0, add 1 (USHD draws use 1-based indexing)
        if(min(draws[, sim]) == 0){
          draws[, sim := sim + 1L]
        }
      } else {
    ...
    #' @param rei_id [integer] rei_id to save.
    #'
    save_draws_and_estimates = function(draws, acause, measure, year, sex, type, race = NULL, edu = NULL, age = NULL, rei_id = NULL) {
      draws <- copy(draws)
      ...
      if (is_1D_raking) {
        # If self$measure is "pred" or "ab", self$std_wt -> self$load_std() is calculated with subsetted ages.
        ...
        cleaned_draws <- calc_all_ages(cleaned_draws, std_wt, "value",
                                       ...
                                       measure = measure, allow_missing_ages = self$by_edu, rei_id = rei_id)
      }
      ...
      # instead of separate save_draws() and save_estimates() calls
      if (type == 'modeling'){
  ...
  active = list(
    #' @field std_wt
    #'  https://r6.r-lib.org/articles/Introduction.html#active-bindings
...
GBDSettings <- R6::R6Class(
  ...
  public = list(
    ...
    load_draws = function(acause, year, sex, measure, population = NULL, raked, age = NULL, measure_id = NULL,
      ...
      # For AB:
      # if metric_id is 3 (rate), pull metric 1 (count) because rate draws are
      # not created in the burdenator, and therefore not accessible from get_draws() 
      if(measure == "ab" && metric_id == 3){
        load_metric <- 1  
      } else {
        load_metric <- metric_id
      }                      
      if ('load_from_dir' %in% names(self$rake_to_gbd_version)){
        message(sprintf("Loading GBD draws from disFILEPATH %s", self$rake_to_gbd_version$load_from_dir))
        # stop if measure is AB -- otherwise, fix for metric_id won't work
        if (identical(measure, "ab")) {
          lsae.utils::stop_or_quit("Cannot load AB from disk (if you get this warning, update handling of metric id in model_settings.r).")
        }
        raw.upper.draws = self$load_from_disk(acause, year, sex, measure, self$ages)
      } else {
        raw.upper.draws = self$load_from_shared_function(acause, year, sex, measure, measure_id, metric_id = load_metric,
                                                         location_id)
      }
      # process_gbd_draws returns data with a 'value' column
      # for the metric argument, pass metric_id (instead of load_metric, as above) b/c this will convert counts to rate, if measure == "ab" & metric_id == 3      
      draws <- self$process_gbd_draws(raw.upper.draws, acause, year, sex, measure, population, raked, age, metric_id) 
    ...
    load_from_disk = function(acause, year, sex, measure, ages){
      ...
      load.draw.file <- function(loc_id) {
        ...
        # USHD models "85+" but Demographics results have 3 groups for that
        ...
        # standardize to USC HD "age" identifiers
    ...
    #' @param raked [character] Ignored for GBD; added to make interface consistent with ModelSettings.
    #' @param age [integer] Age to subset data to.
    #' @param metric [integer] Only required for AB. If metric is 3, convert count --> rate space. Otherwise, do nothing
    #'
    #' @return Draws data.table
    process_gbd_draws = function(raw.upper.draws, acause, year, sex, measure, population, raked, age, metric) {
      causes <- get_ids("cause")
      ...
      # get_gbd_upper_draws returns "ylds", "pred" in rate space, others "mx, yll" in count space
      if (measure %in% c("yld", "pred")) {
        draws[, "value" := deaths]
      } else if (measure == "ab") {
        # Burdenator only produces draws for AB for metric_id 1 and 2. If we want
        # metric_id 3 (rates), need to divide by population.
        # Otherwise, return as-is. The calling script specifies which space to get GBD draws for.
        if (metric == 3) {
      ...
      # note that the process below might cause issues if the order of draws/specific
      # draw index matters (e.g., if need to merge these GBD draws with other draws that 
      # should be aligned at the draw-level). It's an issue b/c sim is a factor column
      # sorted alphabetically, and, e.g., draw_2 > draw_10 in this case.
      draws[, sim := as.integer(sim)] 
