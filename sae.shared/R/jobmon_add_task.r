...
#' @param queue character, default QUEUE Queue for submitting jobs (eg, "QUEUE").
...
#' @param archive boolean, default T. Request node with FILEPATH access?
...
#' @param shell string, default SciComp shell `FILEPATH/execRscript.sh`. Filepath to R shell
...
jobmon_add_task <- function(jobmon_task_template,
                            ...
                            shell = "OMP_NUM_THREADS=1 FILEPATH/execRscript.sh -s", 
                            priority = NULL,
                            sing_image = "default",
                            ...) {
