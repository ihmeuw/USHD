...
##              use_graduation [optional; default = T] - apply graduation to refine ax estimates.
...
set_starting_ax <- function(mx, sex, x, lt_hc, alpha = NULL, beta = NULL, gr, adult_ages_only = FALSE) {
  ...
  if (x == 13 & !adult_ages_only) {
    stop("x is 13, but adult_ages_only is FALSE. if x is 13 then adult_ages_only should be true.")
  }
...
graduation <- function(mx, ax, qx, n, x, adult_ages_only = FALSE) {
  ...
  # identify the age groups graduation will be applied to. Graduation only works
  # for age groups where (n-1), n, and (n+1) all equal 5, so we have to skip the
  # first three age groups (because the first two are 0 and 1-4) and the last
  # two age groups (because the last age group is always the terminal age
  # group). Said another way, graduation only works when n is 5 (which excludes
  # 0, 1-4, and 85+) and won't work on the first and last age groups (which
  # excludes 5-10 and 80). See Preston page 45 for details.
  if (adult_ages_only) {
    # x is the number of age groups
    if (x != 13) {
      stop("x is not 13 while adult_ages_only is TRUE.")
    }
    ii <- 2:(x - 2) # exclude the first age group (age 25) and last two age groups (ages 80, 85+).
  } else {
    ii <- 4:(x - 2) # exclude age groups 1, 2, and 3 (ages 0, 1, and 5), and the last two age groups (ages 80, 85+).
  }
  ...
  while (diff > 0.0001) {
    start_ax <- ax
    ax[ii] <- ((-5/24) * dx[ii - 1] + 2.5 * dx[ii] + (5/24) * dx[ii + 1])/dx[ii] # note that the 5 is just "n" in Preston page 45, and 2.5 is n/2
...
lifetable <- function(mx, ax = NULL, sex = NULL, use_graduation = T, extrap = T, lt_hc, gr = NULL, alpha=1.4, beta=0.095, adult_ages_only = FALSE, skip_mx_0_check = FALSE) {
  ...
  if (is.null(ax)) ax <- set_starting_ax(mx, sex, x, lt_hc, alpha, beta, gr, adult_ages_only = adult_ages_only)
  ...
  if(!(NA %in% ax)) {
    if (use_graduation) c(ax, qx) %<-% graduation(mx, ax, qx, n, x, adult_ages_only = adult_ages_only)
  }
