...
#' @export
touch <- function(path) {
  system(sprintf(SYSTEM_COMMAND), wait = TRUE)
}
...
load_child_cause_metadata <- function(dir, parent_acause = NULL, sex = NULL, age = NULL) {
  ...
  if (!setequal(expected.cause.cols, intersect(expected.cause.cols, colnames(causes)))) {
    lsae.utils::stop_or_quit("causes schema seems to have changed")
  }
...
#' The reason for this is less that saveRDS is bad, and more that eventually we have to persist this data to a database anyway - so we have to write a new save function
#'
...
write_csv <- function(x, file = "", update = FALSE, row.names = TRUE, quote = TRUE, lock = FALSE) {
  ...
  if (lock) {
    ...
    if (length(Sys.glob(file_glob)) > 0) {
      ...
      system(sprintf(SYSTEM_COMMAND), wait = TRUE)
      system(paste0(SYSTEM_COMMAND), wait = TRUE)
    }
  }
  if (update) {
    if (!file.exists(file)) {
...
cause_mx_draws_path <- function(root, acause, measure = "mx", type = "draws", area_var, year, sex, race = race_default, 
  ...
  # In the fall of 2021, the USHD team added 'education' as an additional raking dimension.
  # This meant that all old filepaths would break. To plan for an education rake
  # while preserving backwards compatibility, we added the option 'ushd.use_edu_paths' to the .RProfile. INDIVIDUAL_NAME L 10/28/21
...
get_compiled_estimates_path_ab <- function(root, cause, raked, years, measure_id) {
  paths <- sprintf(
    "%s/est/%s_ab_est_all_%s_%s%s.rds",
    root,
    measure_id,
    cause,
    years,
...
load_compile_estimates_ab <- function(..., verbose = TRUE) {
  ...
  if (verbose) {
    cat("Loading compiled estimates from:\n")
...
#' @param draw_settings [list] named list specifying status, release_id, and version. From settings
...
get_gbd_upper_draws <- function(raking_area_var, cause_id, year, sex, ages, draw_settings, measure = "mx", 
  ...
  # stop if location_ids is null
  if (is.null(location_ids)) {
    stop("location_ids is NULL")
  }
  ...
  if (identical(measure, "yld")) {
    ...
    metric <- metric_id
    # error if metric == 3 -- the Burdenator doesn't save rates, so they're not
    # available from get_draws
    if (metric == 3) {
      stop("metric_id == 3 is not supported for measure == 'ab' (not produced by burdenator)")
    }
    source <- "burdenator"
  } else {
  ...
  # https://stackoverflow.com/a/24590543
...
#' Need to implement draw-checking and downsampling similar to getting GBD draws
#'
