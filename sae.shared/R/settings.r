...
get_settings <- function(dir, dest.env = .GlobalEnv) {
  ...
  optional <- c(
    ...
    "year_knots_num", "year_knots_spec",
    "age_knots_spec","lt_hc",
	...
	# if you want to only model one state
	"subset_state",
	# if you want to only model for one race/ethnicity
	"race_subset", "deaths_file"
  )

  # assign the all-race value so that we don't have to hardcode this
  assign("all_pop_id", race_default, envir = dest.env)

  # if you don't want 2019 and 2020 to be attached in the adjacency matrix, and it is not
  # specified in the settings.csv, set it to FALSE
  if (!"detach_2019_2020" %in% ls(envir = dest.env)) assign("detach_2019_2020", FALSE, envir = dest.env)
  if (!"detach_2020_2021" %in% ls(envir = dest.env)) assign("detach_2020_2021", FALSE, envir = dest.env)
  if (!"detach_2021_2022" %in% ls(envir = dest.env)) assign("detach_2021_2022", FALSE, envir = dest.env)

  if(!"detach_spline_19_21" %in% ls(envir = dest.env)) assign("detach_spline_19_21", FALSE, envir = dest.env)
  if(!"detach_spline_19_22" %in% ls(envir = dest.env)) assign("detach_spline_19_22", FALSE, envir = dest.env)
  ...
  if (!"zero_covar_offset" %in% ls(envir = dest.env)) assign("zero_covar_offset", TRUE, envir = dest.env) # zeroing out the covar offset is normal, so its default value should be TRUE
  ...
  if(!"pop_add1" %in% ls(envir = dest.env)) assign("pop_add1", FALSE, envir = dest.env)
