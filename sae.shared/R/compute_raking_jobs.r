...
compute_raking_jobs <- function(to_dir, from_dir, children_of, initial_level, terminal_level, years, sexes, ages,
  ...
  if (!is.null(children_of)) {
    if (!children_of %in% unique(causes$acause)) {
      stop_or_quit(paste0("children_of argument is invalid! Was passed: ", children_of), status = 2)
    }
  ...
  #   * compile jobs are in perspective of a single cause for which results are compiled
  ...
  # Job names: detailed by all relevant demographics
  # All-cause raking not currently run by age; cause-specific raking is
  # Ex: for raking to _inj for 2000, males, age 0 job name is 'rake__inj_2000_1_0'
  ...
  # subset raking jobs to initial/terminal level.
  valid.range <- seq(initial_level - 1, terminal_level)
