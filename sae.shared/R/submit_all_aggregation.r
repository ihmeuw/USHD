...
#'                "post_raking" -- Using raked area-year-sex-race ASMRs only
#'                "post_raking_lt" -- Run Life_tables (for _all) then rake area-year-sex-race ASMRs
#'                "post_raking_lt_yll" -- Run Life_tables (for _all), then pred_yll then rake area-year-sex-race ASMRs
...
#' @param skip_yld_aggregation logical -- skip yld aggregation? Default F
#' @param skip_pred_aggregation logical -- skip pred aggregation? Default F
...
submit_all_aggregation <- function(jobmon_workflow,
                                   ...
                                   skip_pred_aggregation = F,
                                   initial_level = NULL,
                                   terminal_level = NULL,
                                   children_of = NULL) {

  stopifnot(grepl("mortality/sae_models$", getwd()))
  stopifnot(type %in% c("post_raking", "agg_raked_yll", "post_raking_lt", "post_raking_lt_yll"))

  validate_cause_subset_args(children_of, initial_level, terminal_level)

  message("\nSettings passed to submit_all_aggregation:")
  arguments <- c("dir", "type", "testing", "project", "queue", "skip_mx_aggregation", "skip_yll_aggregation", "skip_yld_aggregation", "skip_pred_aggregation",
  ...
  for (this_cause in causes[order(level, cause_id), cause_id]) {
    ...
    message("\nCalling submit_single_aggregation for ", cause_name)
    output <- submit_single_aggregation(jobmon_workflow = jobmon_workflow_holder,
                                        ...
                                        lt_hold = if (cause_name == "_all" | skip_mx_aggregation) NULL else lt_jids) # lt_holds only needed for cause specific mx aggregation
