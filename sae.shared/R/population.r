...
#' @param draw_settings list with at least release_id value
...
validate_ushd_pop_to_gbd <- function(population, draw_settings, pop_file = "not given", tol = 0.1) {
  ...
  if (85 %in% ages) {
    ...
    if (draw_settings$release_id > 10) {
      warning(
        "Assuming 85+ corresponds to GBD age groups 31, 32, and 235, which may not be ",
        "true for GBD rounds past GBD 2020 (round 7, release_id 10). Please confirm this."
      )
    }
  ...
  gbd_population <- get_population(
    ...
    release_id = draw_settings$release_id,
  ...
  if (nrow(gbd_population) == 0) {
    stop(paste0(
      "No GBD population found for release_id ", draw_settings$release_id,
