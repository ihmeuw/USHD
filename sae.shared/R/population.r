...
#' @param skip_tiny_pop_correction: If TRUE, do not set populations less than 1e-5 to 0. Default is FALSE.
#'
#' @export
add.population.weights <- function(data, population, demographics, weight_var, area_var, skip_tiny_pop_correction = FALSE) {
  ...
  # calculate aggregation weights. include fallbacks where a given area_var/year/sex/age is 0. The
  # numerator would be whatever the rows of pop are. If the population file is race specific, then
  # the numerator includes race. So in the case of a race rake, the numerator would be
  # year,age,sex,sim,mcnty,race, and then the denominator would be year,age,sex,sim,mcnty
  ...
  if (any(nans)) {
    ...
    pop_demographics <- c(fallback_demographics[fallback_demographics %in% names(population)], if(by_race) "race", if(by_edu) "edu") # NOTE: race/edu would already have been used in the main weights, due to the structure of population file.
    population[, .tmp.pop := sum(pop), by = pop_demographics] # numerator
    # example: numerator is "year", "age", "mcnty", "race" and denominator is "year", "age", "sex", "mcnty". The only difference is that sex was removed from the numerators
    ...
    nans <- is.nan(data[[weight_var]]) # Need to reassign mask as indices are changed on merge. NOTE that this is still just NaNs in the weight_var column
    filled_nans <- nans & !is.nan(data[[".tmp.weight"]]) # This new mask represents rows where weight_var is NaN, but the new .tmp.weight is not NaN.

    warning(sprintf("Setting %i rows to fallback weight (%s)", sum(filled_nans), paste(pop_demographics, collapse = "/")))
    data[filled_nans, get("weight_var") := .tmp.weight] # using the new mask, replace weight var (NaN) with .tmp.weight (not NaN).
    data[, .tmp.weight := NULL] # clean up temporary variable

    # repeat the process, but at a less detailed level: removing not only sex, but age as well.
    nans <- is.nan(data[[weight_var]])
    if (any(nans)) {
...
validate_ushd_pop_to_gbd <- function(population, draw_settings, pop_file = "not given", tol = 0.1) {
  ...
  # USHD age group 85+'s GBD age group id (160) does not have values in the database
  # so we have to manually calculate it
  if (85 %in% ages) {
